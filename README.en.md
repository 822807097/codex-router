...
| **ChatGPT web-pool native tool mode (experimental)** | Web-pool accounts can switch to the native tool path via the official connector + secure tunnel: web search / computer control reach the web session as real tool calls, with screenshot re-injection and per-account switches. Off by default; the text-protocol path always remains as automatic fallback (see [docs/ADVANCED.en.md](docs/ADVANCED.en.md)) |
...
**Q: What is the web pool's "native tool mode"? Why is it off by default, and how do I enable it?**
