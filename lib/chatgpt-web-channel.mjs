...
export async function collectToolResultImages({ messages = [], bridge = null, log = () => {} } = {}) {
  ...
  for (const url of urls.slice(0, MAX_UPLOAD_IMAGES_PER_REQUEST)) {
    try {
      ...
      log({ event: 'chatgpt_web.upload_failed', outcome: 'fail', message: String(error?.message || error).slice(0, 160) });
    }
  }
  return assets;
}
...
export async function openChatGptWebChatStream({
  ...
  for (let attemptNo = 0; attemptNo < maxAccountAttempts; attemptNo += 1) {
    ...
    try {
      ...
      if (!nativeMode) {
        const fileBridge = createWebpoolFileBridge({ accessToken: credentials.accessToken, fp, proxy, log, request: openStream });
