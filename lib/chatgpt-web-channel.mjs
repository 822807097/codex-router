...
import {
  parseDataUrl,
  buildAssetPointerPart,
  createWebpoolFileBridge,
} from './chatgpt-web-files.mjs';
import {
...
// ---------- 原生模式判定与会话降级（治根改造 P3） ----------
// 原生 = OpenAI 后端经账号连接器编排工具（工具执行在路由门面侧，见 webpool-mcp-server）。
// 门控：全局 chatgptWeb.nativeTools.enabled 且该账号 metadata.nativeTools===true
// （逐账号显式开启——个人账号写能力边界实弹前不默认启用）。
// 降级阶梯：原生回答出现「空/工具不可用泄漏」→ 该会话标记降级，后续请求回文本协议
// （会话身份 = 首条 user 消息内容哈希，TTL 30 分钟，容量 2000 防无界增长）。
const NATIVE_DOWNGRADE_TTL_MS = 30 * 60_000;
const nativeDowngrades = new Map(); // sessionKey -> expiresAt

export function webpoolSessionKey(messages) {
  const list = Array.isArray(messages) ? messages : [];
  const firstUser = list.find((m) => m?.role === 'user');
  const raw = typeof firstUser?.content === 'string'
    ? firstUser.content
    : JSON.stringify(firstUser?.content ?? '');
  return createHash('sha256').update(String(raw).slice(0, 4000)).digest('hex').slice(0, 16);
}

function isNativeDowngraded(key) {
  const expiresAt = nativeDowngrades.get(key);
  if (expiresAt === undefined) return false;
  if (Date.now() > expiresAt) { nativeDowngrades.delete(key); return false; }
  return true;
}

export function markNativeDowngraded(key) {
  if (nativeDowngrades.size >= 2000) {
    const oldest = nativeDowngrades.keys().next().value;
    nativeDowngrades.delete(oldest);
  }
  nativeDowngrades.set(key, Date.now() + NATIVE_DOWNGRADE_TTL_MS);
}

export function isNativeToolsMode({ account = null, config = null, messages = [] } = {}) {
  if (config?.enabled !== true) return false;
  if (account?.metadata?.nativeTools !== true) return false;
  return !isNativeDowngraded(webpoolSessionKey(messages));
}

export function resetWebpoolNativeStateForTests() {
  nativeDowngrades.clear();
  webpoolFileBridges.clear(); // 桥跨轮复用含 sha 缓存，测试隔离必须一并清（否则上一用例缓存命中伪造上传成功）
}

// P1-2：桥按「账号+token」跨轮复用——30 分钟 sha 缓存才能命中（每请求新建=同批截图每轮重传）
const webpoolFileBridges = new Map();

function getWebpoolFileBridge(key, factory) {
  let bridge = webpoolFileBridges.get(key);
  if (!bridge) {
    if (webpoolFileBridges.size >= 8) webpoolFileBridges.delete(webpoolFileBridges.keys().next().value);
    bridge = factory();
    webpoolFileBridges.set(key, bridge);
  }
  return bridge;
}
...
export function chatMessagesToConversationMessages(messages = [], opts = {}) {
  const systemParts = [];
  ...
  for (let index = 0; index < messages.length; index += 1) {
    ...
    if (item?.role === 'assistant' && Array.isArray(item?.tool_calls) && item.tool_calls.length) {
      ...
      if (turn) {
        ...
        // P4.1 截图回注：data URL 已经在组包前换成会话文件指针 → multimodal parts；
        // 上传失败的图保持纯文本（模型看不见但任务不被拖死）
        const pointers = (turn.images || [])
          .map((url) => opts?.imageAssets?.get(url))
          .filter((r) => r && r.fileId);
        conversationMessages.push(pointers.length
          ? {
            id: randomUUID(),
            author: { role: 'user' },
            content: {
              ...
              parts: [
                { content_type: 'text', text: turn.userText },
                ...pointers.map((p) => buildAssetPointerPart(p)),
              ],
            },
          }
          : {
  ...
  if (conversationMessages.length > MAX_WEB_HISTORY_MESSAGES) {
    ...
    const headTextAt = (i) => {
      const head = conversationMessages[i]?.content?.parts?.[0];
      return typeof head === 'string' ? head : String(head?.text || '');
    };
    while (
      start > 1
      && start < conversationMessages.length
      && headTextAt(start).startsWith('工具结果：')
    ) {
      start -= 1;
    }
...
/** 组包前预上传工具结果截图（P4.1）：data URL → 会话文件指针；失败仅日志不阻断。 */
const MAX_UPLOAD_IMAGES_PER_REQUEST = 6;

export async function collectToolResultImages({ messages = [], bridge = null, log = () => {} } = {}) {
  const assets = new Map();
  if (!bridge || typeof bridge.upload !== 'function') return assets;
  const urls = [];
  const seen = new Set();
  for (const msg of Array.isArray(messages) ? messages : []) {
    ...
    for (const part of msg.content) {
      ...
      if (url.startsWith('data:image/') && !seen.has(url)) { seen.add(url); urls.push(url); }
    }
  }
  // P1-1：并行上传（单图失败互不拖累），abort/预算由 bridge 的 signal 生效
  const targets = urls.slice(0, MAX_UPLOAD_IMAGES_PER_REQUEST);
  const results = await Promise.allSettled(targets.map(async (url) => {
    const parsed = parseDataUrl(url);
    if (!parsed) return;
    const up = await bridge.upload({ bytes: parsed.bytes, mime: parsed.mime });
    if (up?.fileId) assets.set(url, { fileId: up.fileId, width: up.width, height: up.height, sizeBytes: parsed.bytes.length });
  }));
  for (const result of results) {
    if (result.status === 'rejected') {
      log({ event: 'chatgpt_web.upload_failed', outcome: 'fail', message: String(result.reason?.message || result.reason).slice(0, 160) });
    }
  }
  return assets;
}

export function buildConversationPayload(messages, model, timezone = 'Asia/Shanghai', tools = null, opts = {}) {
  ...
  const conversation = {
    action: 'next',
    messages: chatMessagesToConversationMessages(cleanedMessages, opts),
    model,
...
export async function openChatGptWebChatStream({
  ...
  timeouts,
  log = () => {},
  // 网页池 CU 工具注入开关（config.json tools.webPoolInject，默认开；
  // 环境变量 CHATGPT_WEB_MCP_INJECT=0 优先级更高的逃生门）
  webPoolMcpInject,
  // 原生模式全局开关（config.json chatgptWeb.nativeTools；逐账号还需 metadata.nativeTools）
  nativeToolsConfig = null,
} = {}) {
  ...
  for (let attemptNo = 0; attemptNo < maxAccountAttempts; attemptNo += 1) {
    ...
    try {
      ...
      // 原生模式（治根 P3）：全局开关+该账号显式开启且会话未降级 → 完全跳过文本协议
      // （工具编排由 OpenAI 后端经账号连接器完成，工具执行在路由门面侧）。
      // 降级阶梯：原生回答为空或泄漏「工具不可用」→ 该会话回文本协议（TTL 30 分钟）。
      const sessionKey = webpoolSessionKey(chatBody?.messages);
      const nativeMode = isNativeToolsMode({ account, config: nativeToolsConfig, messages: chatBody?.messages });
      const nativeGuard = nativeMode
        ? (finalText) => {
          if (!String(finalText || '').trim() || detectToolRefusal(finalText)) {
            markNativeDowngraded(sessionKey);
            log({ event: 'webpool.native_fallback', outcome: 'refusal_or_empty', message: `session=${sessionKey} account=${account.id}` });
          }
        }
        : null;
      // 工具面统一：注入 CU 目录后的工具表同时喂给组包（提示）与解析（allowedNames）——
      // 只喂提示会让模型输出的调用被解析器当未知名丢弃（2026-09-10 实测最后一环断裂）
      const effectiveTools = nativeMode
        ? []
        : mergeContinuationMcpTools(
          Array.isArray(chatBody?.messages) ? chatBody.messages : [],
          Array.isArray(chatBody?.tools) ? chatBody.tools : null,
          { inject: webPoolMcpInject !== false },
        );
      log({
        event: 'chatgpt_web.tools_effective',
        model: modelSlug,
        outcome: nativeMode ? 'native' : 'text_protocol',
      ...
      // P4.1 截图回注（仅文本协议路径）：组包前把工具结果里的 data URL 截图走
      // 三步上传换成会话文件指针；原生模式不回灌——工具执行在门面侧，无水截图可传。
      // request 缝与 conversation 派发同源：注入 openStream 时测试/特殊通道可拦。
      // P1-2：桥按「账号+token」跨轮复用，sha 缓存才能命中（每请求新建=同批截图反复上传）；
      // P1-1：上传挂客户端 abort + 12s 总预算，超时放弃剩余图不拖死响应。
      ...
      if (!nativeMode) {
        ...
        const bridgeKey = `${account.id}|${credentials.accessToken}`;
        const fileBridge = getWebpoolFileBridge(bridgeKey, () => createWebpoolFileBridge({
          accessToken: credentials.accessToken, fp, proxy, log, request: openStream, signal: uploadSignal,
        }));
        imageAssets = await collectToolResultImages({ messages: chatBody?.messages, bridge: fileBridge, log });
      }
      ...
      if (status === 200) {
        ...
        if (chatBody?.stream === false) {
          ...
          const completion = conversationSseToChatCompletion(sseText, {
            model: modelSlug,
            tools: effectiveTools,
            onTextFinal: nativeGuard || undefined,
          });
          return {
            status: 200,
            headers: { 'content-type': 'application/json', 'x-web-bridge-version': nativeMode ? 'v25-native-guard' : 'v24-narration-strip' },
        ...
        return { status: 200, headers: { 'content-type': 'text/event-stream', 'x-web-bridge-version': nativeMode ? 'v25-native-guard' : 'v24-narration-strip' }, stream: upstream.stream.pipe(transform) };
      }
      ...
      if (status === 401) {
        ...
        // 池内全 401 时按 401 语义回客户端：智能体客户端对 401 停止重试、
        // 对 502 退避重试，语义错位会白烧重试预算（2026-09-13 审计）
        failure.status = 401;
