...
class ChatSseToResponsesTransform extends Transform {
  ...
  appendToolCall(fragment, fallbackIndex) {
    ...
    // 双向归并（2026-09-13 审查 P2）：首帧只有 id 没有 index、后续帧补 index 时，
    // `id:X` 与 `index:N` 是两个不同键——不归并会把同一调用拆成两个（首调参数
    // 截断 + 幽灵 tool 调用）。两个方向都只在「候选唯一」时归并，并行多调用
    // 场景无锚点不可靠关联，宁可分裂也不误并。
    if (key.startsWith('index:') && !this.tools.has(key)) {
      const unclaimed = [...this.tools.entries()]
        .filter(([existingKey, tool]) => existingKey.startsWith('id:') && !tool.claimedByIndexKey);
      if (unclaimed.length === 1) {
        const [oldKey, adopted] = unclaimed[0];
        this.tools.delete(oldKey);
        adopted.claimedByIndexKey = key;
        this.tools.set(key, adopted);
      }
    } else if (key.startsWith('id:') && !this.tools.has(key)) {
      // 反向（首帧 index、后续帧补 id）：callId 是随机生成（未见过真实上游 id）
      // 的唯一条目可被认领——否则新 id 帧会分裂出新条目
      const adoptable = [...this.tools.entries()]
        .filter(([, tool]) => !tool.sawUpstreamId && !tool.callIdAdopted);
      if (adoptable.length === 1) {
        const [oldKey, adopted] = adoptable[0];
        this.tools.delete(oldKey);
        adopted.callId = fragment.id;
        adopted.callIdAdopted = true;
        this.tools.set(key, adopted);
      }
    }
    ...
    if (!tool) {
      ...
      tool = {
        ...
        // 该条目是否已见过真实上游 id（反向归并的候选条件）
        sawUpstreamId: Boolean(fragment?.id),
      };
      this.tools.set(key, tool);
    }
    if (!tool.emitted && fragment?.id) {
      tool.callId = fragment.id;
      tool.sawUpstreamId = true;
    }
...
class ResponsesSseToChatTransform extends Transform {
  constructor(model, options = {}) {
    ...
    // message 正文已发字符数（done-only 网关的差额补发记账，与 summaryEmitted 对称）
    this.messageTextEmitted = 0;
    this.usage = undefined;
    this.finishReason = null;
  ...
  processPayload(payload) {
    ...
    if (type === 'response.output_text.delta') {
      ...
      this.messageTextEmitted += delta.length;
    ...
    if (type === 'response.output_item.done') {
      ...
      // message 终态补齐（与 reasoning 摘要的 done 补发对称）：部分第三方
      // Responses 兼容层只发 added+done 不发 output_text.delta——不补发的话
      // chat 客户端收到 200 + 空正文（2026-09-13 审查 P2）
      if (item?.type === 'message' && Array.isArray(item.content)) {
        const doneText = item.content
          .map((part) => (part?.type === 'output_text' && typeof part.text === 'string' ? part.text : ''))
          .join('');
        if (doneText && doneText.length > this.messageTextEmitted) {
          const missing = doneText.slice(this.messageTextEmitted);
          this.messageTextEmitted = doneText.length;
          if (this.reserveAccumulatedBytes(missing)) this.appendText(missing);
        }
      }
      return;
    }
...
class ExecCustomToolBridgeTransform extends Transform {
  constructor(options = {}) {
    ...
    this.rewrote = 0;
    // 已被改写为 custom_tool_call 的 item_id 集合：参数增量/完成帧只对这些 item
    // 改名——此前无条件改写所有 function_call_arguments 帧，非 exec 工具的调用
    // 会变成「item 是 function_call、参数帧是 custom input」的类型错配，桌面端
    // 丢参数（2026-09-13 对抗审查）
  ...
  rewriteEvent(decodedEvent) {
    ...
    if (type === 'response.output_item.added' || type === 'response.output_item.done') {
      ...
      // 只有带 id 的 item 才改写：id 是参数帧归并的锚点，缺失时改写 item
      // 而参数帧不动会产生类型撕裂（2026-09-13 审查 P2）
      if (item?.type === 'function_call' && item.id && EXEC_BRIDGE_TOOL_NAMES.has(item.name)) {
        ...
        this.customItemIds.add(rewritten.id);
        this.rewrote += 1;
    ...
    if (type === 'response.function_call_arguments.delta') {
      if (!this.customItemIds.has(payload.item_id)) return null;
      payload.type = 'response.custom_tool_call_input.delta';
      // event 行与 data.type 同步改写：按 event 名分发的客户端（桌面端）会错配
      return { event: 'response.custom_tool_call_input.delta', data: JSON.stringify(payload) };
    }
    if (type === 'response.function_call_arguments.done') {
      if (!this.customItemIds.has(payload.item_id)) return null;
      payload.type = 'response.custom_tool_call_input.done';
      // 字段名同步 custom 协议：arguments → input（桌面端按 input 聚合）
      if ('arguments' in payload) {
        payload.input = payload.input ?? unwrapCustomToolInput(payload.arguments);
        delete payload.arguments;
      }
      return { event: 'response.custom_tool_call_input.done', data: JSON.stringify(payload) };
    }
    return null;
  }
