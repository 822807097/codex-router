...
// reasoning / assistant 正文项是「轮次分隔符」而非调用链中断：思考模型每轮真实
// 形状是 [reasoning, 正文, 调用, 输出]，遇它们 break 会让检测在思考模型上
// 永远落空（2026-09-13 审查 P1：DeepSeek-v4/GLM/Qwen thinking 全裸奔）
const calls = [];
for (let i = compactedInput.length - 1; i >= 0; i -= 1) {
  ...
  if (typeof item?.type === 'string' && item.type.startsWith('message')) continue; // assistant/user 正文
  break;
}
...
export function detectTrailingLoopChat(messages) {
  ...
  // 纯文本 assistant 消息（含思考模型的 reasoning_content/正文）是轮次分隔符，
  // 不应中断收集（与 detectTrailingLoop 的 reasoning 跳过对称）
  const calls = [];
  for (let i = messages.length - 1; i >= 0 && calls.length < 12; i -= 1) {
    ...
    if (message?.role === 'assistant') {
      ...
      continue; // 无 tool_calls 的 assistant 文本/思考消息：继续向前收集
    }
...
export function upstreamModel(target, requestedModel) {
  if (target.modelMap?.[requestedModel]) return target.modelMap[requestedModel];
  let base = target.upstreamModel || target.model || requestedModel;
  // upstreamModel 中的 $N = target.match 对应捕获组（如 match `^web-(.+)$` +
  // upstreamModel '$1'：一条 target 承载整族 web-* 模型，不受 modelMap ≤32 条限制）
  if (base !== requestedModel && /\$\d/.test(String(base)) && target.match && typeof requestedModel === 'string') {
    try {
      const m = requestedModel.match(new RegExp(target.match));
      if (m) {
        base = base.replace(/\$(\d)/g, (whole, idx) => {
          const captured = m[Number(idx)];
          return captured === undefined ? '' : captured;
        });
      }
    } catch { /* target.match 非法时按原样回落 */ }
    // 替换失败仍带占位符：宁可不映射也不能把 '$1' 当模型名发给上游
    if (/\$\d/.test(String(base))) return requestedModel;
  }
  return base || requestedModel;
}
