...
export function updateModelCatalogFile(catalogPath, modelContext, options = {}) {
  ...
  // 唯一临时名：固定 `.tmp` 在两次并发写互踩（生产主路径已走 revision CAS，
  // 此入口当前无生产调用方，保留为最小安全实现）
  const tempPath = `${catalogPath}.tmp-${process.pid}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
  try {
    fileSystem.writeFileSync(tempPath, JSON.stringify(result.catalog, null, 2));
    fileSystem.renameSync(tempPath, catalogPath);
  } catch (error) {
    try { if (fileSystem.existsSync(tempPath)) fileSystem.unlinkSync(tempPath); } catch { /* 清理失败不掩盖原错误 */ }
    throw error;
  }
  return result;
}
