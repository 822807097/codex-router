...
export function responsesToChatMessages(input, options = {}) {
  ...
  // to the API"（2026-09-11 deepseek-chat 通道实锤，真实 96 消息会话复现：只要思考
  // 会话里存在任何缺该字段的 assistant 消息，整条请求被拒——单条缺失最小复现反而不
  // 触发，是会话级一致性校验）。
  // 桌面端每轮的真实形状是 [reasoning, assistant 正文, 工具调用, 输出]，DeepSeek 原生
  // 形态把三者合成一条 assistant 消息（content=正文、reasoning_content=思考、
  // tool_calls=调用）。这里同样合并；思考会话中给缺失的 assistant 消息补空串，
  // 保证字段全覆盖。passBackReasoning=false 对不兼容上游整体关闭。
  let pendingReasoning = '';
  let pendingAssistantText = [];
  let sawReasoning = false;
  const takePendingReasoning = () => {
    if (options.passBackReasoning === false || !pendingReasoning) {
      if (options.passBackReasoning !== false && sawReasoning) return '';
      return undefined;
    }
  ...
  const flushTools = () => {
    ...
    const message = { role: 'assistant', content: takeAssistantText(), tool_calls: pendingToolCalls };
  ...
  const flushAssistantText = () => {
    if (pendingToolCalls.length || !pendingAssistantText.length) return;
    const message = { role: 'assistant', content: takeAssistantText() };
    const reasoning = takePendingReasoning();
    if (reasoning !== undefined) message.reasoning_content = reasoning;
    dialogue.push(message);
  };
  ...
  const flushNotes = () => {
    ...
    const message = { role: 'assistant', content: pendingNotes.join('\n') };
    if (sawReasoning && options.passBackReasoning !== false) message.reasoning_content = takePendingReasoning() || '';
    dialogue.push(message);
    pendingNotes = [];
  };
  ...
  for (const item of items) {
    ...
    if (item.role === 'developer' || item.role === 'system') {
      ...
      // 同轮正文缓存：紧随的 reasoning/工具调用会在 flushTools 合并成单条
      // assistant 消息（DeepSeek 原生形态）；无工具调用时由边界 flush 成纯正文。
      // 先 flushNotes：正文之前的官方调用注记必须保持在其前（时序语义）。
      flushNotes();
      const text = extractText(item.content);
      ...
      // 不 flush 缓冲正文：同轮的 assistant 正文要并入即将生成的工具调用消息
      //（DeepSeek 原生形态：content+reasoning_content+tool_calls 同条）。
      flushNotes();
