...
import { panelBuildStatus } from './panel-build-info.mjs';
import { probeMcpServer } from './tool-bridge.mjs';
...
import { fetchChatGptWebModels, testChatGptWebModel } from './chatgpt-web-channel.mjs';
...
export function createAdminHandler(options) {
  ...
  function syncCodexConfigEnvKey(enable) {
    ...
    if (enable) {
      ...
      if (authLine !== -1 && /^\s*requires_openai_auth\s*=\s*true\s*$/.test(lines[authLine])) {
        lines[authLine] = lines[authLine].replace(/=\s*true\s*$/, '= false');
      ...
      if (authLine !== -1 && /^\s*requires_openai_auth\s*=\s*false\s*$/.test(lines[authLine])) {
        lines[authLine] = lines[authLine].replace(/=\s*false\s*$/, '= true');
  ...
  function setRouterEnvKeyVariable(key) {
    ...
    return `非 Windows 平台请手工 export ROUTER_API_KEY（如写入 ~/.zshrc 或 ~/.bashrc 后 source 生效）`;
  }
  ...
  function persistAuthorizedAccount({ provider, email, planType, projectId, organizationUuid, credentials, extraMetadata = {} }) {
    ...
    const alias = safeEmail
      ? `${provider === 'google' ? 'Google' : provider === 'openai' ? 'ChatGPT' : provider === 'chatgpt-web' ? 'ChatGPT网页' : 'Claude'} (${safeEmail})`
      : `${provider} account`;
  ...
  async function completeAuthorization(provider, { code, codeVerifier, redirectUri }) {
    ...
    // chatgpt-web 与 openai 同一令牌交换机制（差异只在落库 provider 名）
    if (provider === 'openai' || provider === 'chatgpt-web') {
  ...
  function statusBody() {
    return {
      ...
      // 版本自描述（面板/排障用；此前设置页版本号只能靠查 GitHub 更新接口）
  ...
  async function handleApi(req, res, url) {
    ...
    // ---------- 网页池原生 MCP 模式（治根改造 P1b） ----------
    if (req.method === 'GET' && url === '/_admin/api/webpool/native-status') {
      const wn = options.webpoolNative;
      const cfgView = wn?.config ? {
        enabled: wn.config.enabled,
        port: wn.config.port,
        exposeComputer: wn.config.exposeComputer,
        exposeWebSearch: wn.config.exposeWebSearch,
        bearerKey: wn.config.bearerKey, // 只暴露凭据「变量名」，值永不出面
      } : null;
      jsonResponse(res, 200, {
        configured: Boolean(wn),
        config: cfgView,
        mcp: wn?.mcpStatus?.() || null,
        catalog: wn?.catalogStatus?.() || null,
        tunnel: wn?.tunnelStatus?.() || null, // P2 隧道接入后填充
      });
      return true;
    }
    // P5 原生模式指标（内存滑动聚合，只读）：订阅页原生工具模式卡片消费
    if (req.method === 'GET' && url === '/_admin/api/webpool/metrics') {
      jsonResponse(res, 200, options.webpoolMetrics
        ? options.webpoolMetrics.snapshot()
        : { days: [], totals: {}, updatedAt: 0 });
      return true;
    }
    if (req.method === 'POST' && url === '/_admin/api/webpool/native-selftest') {
      const wn = options.webpoolNative;
      if (!wn?.selfTest) { jsonResponse(res, 200, { ok: false, error: '原生门面未接线（版本过旧或启动异常）' }); return true; }
      try {
        const result = await wn.selfTest();
        options.log?.({ event: 'webpool.native_selftest', ok: result?.ok === true, message: String(result?.error || `${result?.toolCount ?? 0} 工具`).slice(0, 200) });
        jsonResponse(res, 200, result);
      } catch (error) {
        options.log?.({ event: 'webpool.native_selftest', ok: false, message: String(error?.message || error).slice(0, 200) });
        jsonResponse(res, 200, { ok: false, error: String(error?.message || error).slice(0, 200) });
      }
      return true;
    }
    // 逐账号原生模式开关（P3 灰度面：先单账号实弹，再逐步铺开）
    if (req.method === 'POST' && url === '/_admin/api/webpool/accounts/native-tools') {
      const payload = await readJsonBody(req);
      const accountId = String(payload?.account || payload?.id || '');
      const enabled = payload?.enabled === true;
      if (!accountId || !options.authManager?.listAccounts) {
        jsonResponse(res, 400, { ok: false, error: '缺少 account 参数' });
        return true;
      }
      try {
        const accounts = options.authManager.listAccounts();
        const acct = (Array.isArray(accounts) ? accounts : []).find((a) => String(a?.id) === accountId);
        if (!acct) { jsonResponse(res, 404, { ok: false, error: '账号不存在' }); return true; }
        if (acct.provider !== 'chatgpt-web') {
          jsonResponse(res, 400, { ok: false, error: '原生工具开关仅适用于 ChatGPT 网页账号' });
          return true;
        }
        options.authManager.updateAccount(accountId, {
          metadata: { ...(acct.metadata || {}), nativeTools: enabled },
        });
        // P1-3：与 set-proxy/set-priority 同范式——内存态之后必须落库，否则重启静默回退
        try {
          await options.authManager.persistAccount?.(accountId);
        } catch (persistError) {
          jsonResponse(res, 500, { ok: false, error: `开关已生效但持久化失败：${String(persistError?.message || persistError).slice(0, 120)}` });
          return true;
        }
        const globalOn = options.webpoolNative?.config?.enabled === true;
        jsonResponse(res, 200, {
          ok: true,
          account: accountId,
          nativeTools: enabled,
          note: enabled && !globalOn
            ? '账号已开启，但全局原生开关未启用（config chatgptWeb.nativeTools.enabled）——需配置后重启路由才会走原生链路'
            : enabled
              ? '该账号后续会话将走原生连接器链路（需已在网页端为它绑定连接器）'
              : '该账号回到文本协议链路',
        });
      } catch (error) {
        jsonResponse(res, 500, { ok: false, error: String(error?.message || error).slice(0, 160) });
      }
      return true;
    }
    ...
    if (req.method === 'PUT' && url === '/_admin/api/model-routing') {
      ...
      try {
        ...
        // 写入纳入 codexDesktopMutex：与 apply-router / restore-official 并发时
        // 裸写会互相覆盖桌面目录文件（2026-09-13 审计）。
        const deletedModels = Array.isArray(plan.impact?.models?.deleted) ? plan.impact.models.deleted : [];
        if (deletedModels.length > 0) {
          const desktopRelease = await codexDesktopMutex.acquire();
          try {
    ...
    if (req.method === 'POST' && url === '/_admin/api/vendor-presets/activate') {
      ...
      if (addCatalog && customModels && customModels.length > 0) {
        ...
        if (needsExtension.length > 0 && targetObj) {
          ...
          const original = typeof targetObj.match === 'string' ? targetObj.match.trim() : '';
          // 原有 match 为空时直接用扩展式；绝不能拼出 `|^(?:...)$`——首分支为空
          // 的正则可匹配任意模型名，等于通道劫持全部路由（2026-09-13 审计）
          if (!original) {
            targetObj.match = extension;
            changes.push(`已设置通道 match 覆盖 ${needsExtension.length} 个自定义模型名`);
          } else if (!original.includes(extension)) {
    ...
    if (url.startsWith('/_admin/api/oauth/')) {
      ...
      if (req.method === 'POST' && action === 'start') {
        ...
        // chatgpt-web（网页会话通道）与 openai 订阅同一 auth.openai.com 令牌体系，
        // 完整复用 loopback OAuth 机制；会话键按实际 provider，账号落库也用实际 provider。
        if (provider === 'openai' || provider === 'chatgpt-web') {
          resetOAuthSession(provider);
          let server = null;
          let mode = 'loopback';
          ...
          oauthSessions.set(provider, session);
          if (server) {
            session.ttlTimer = setTimeout(() => resetOAuthSession(provider), OAUTH_SESSION_TTL_MS);
            session.ttlTimer.unref?.();
            driveLoopbackCompletion(provider, session);
          }
          if (payload?.openBrowser !== false) openDefaultBrowser(authUrl);
      ...
      if (req.method === 'POST' && action === 'exchange') {
        ...
        const redirectUri = session.redirectUri || (provider === 'openai' || provider === 'chatgpt-web' ? 'http://localhost:1455/auth/callback' : undefined);
        try {
    ...
    if (req.method === 'POST' && url === '/_admin/api/accounts/add') {
      ...
      // 代理对齐 OAuth 路径（2026-09-04 开源用户反馈）：仅显式配置才启用——
      // 写死 10808 会让无本地代理客户端的账号出站全部 ECONNREFUSED。
      dbSaveAccount({
        id: accountId,
        provider: account.provider,
        email: account.email,
        alias: account.alias,
        proxy_enabled: payload.proxy?.enabled === true && payload.proxy?.url ? 1 : 0,
        proxy_url: typeof payload.proxy?.url === 'string' ? payload.proxy.url : '',
    ...
    if (req.method === 'POST' && url === '/_admin/api/accounts/delete') {
      ...
      if (!accountId) {
        jsonResponse(res, 400, { error: { code: 'invalid_account', message: '缺少账号 id' } });
        return true;
      }
      if (!authManager?.getAccount(accountId)) {
        jsonResponse(res, 404, { error: { code: 'account_not_found', message: '账号不存在（可能已删除）' } });
        return true;
      }
      authManager.removeAccount(accountId);
      try { dbDeleteAccount(accountId); } catch { /* 容错 */ }
      options.credentialsVault?.delete(accountId);
    ...
    if (req.method === 'POST' && url === '/_admin/api/accounts/fetch-models') {
      ...
      try {
        ...
        } else if (account.provider === 'chatgpt-web') {
          // 网页会话通道：GET /backend-api/models（真实上游清单，无需降级兜底——
          // 失败时如实报错让用户重试，避免内置清单过期误导）
        ...
        // 拉到即入库：新 slug 追加进目录池（models.json/catalogPath），已有条目保留
        // 原值——上游发布新模型（如 gpt-6-astra）后点一次「拉取」即可出现在
        // 「分组自定义模型」与接入弹窗，不必等 apply-router，也不用改代码。
        let addedSlugs = [];
        // 入库告警透出给前端（此前损坏/冲突只进服务端日志，用户毫无感知）
        let syncWarning = '';
        try {
          if (Array.isArray(models) && models.length && options.catalogPath) {
            const fresh = [];
            // 目录池写入必须走 revision CAS（与 PUT /model-routing 同轨）：
            // 裸 read→modify→write 会与模型管理页并发写互踩（旧快照整体覆盖丢更新）。
            // 本端点位于 handler 体前段：不能引用后段才初始化的 const（readPoolCatalog
            // 等，TDZ 实锤 2026-09-06）——此处直接读目录池文件。
            let poolModels = [];
            let catalogRevision = null;
            if (fs.existsSync(options.catalogPath)) {
              try {
                const revisioned = readRevisionedJson(options.catalogPath, { maxBytes: MAX_CATALOG_BYTES });
                if (Array.isArray(revisioned.value?.models)) poolModels = revisioned.value.models;
                catalogRevision = revisioned.revision;
              } catch {
                // 池文件损坏：绝不能按空池重建（会抹掉整个池）——本次只回显清单不入库
                syncWarning = '目录池文件不可读，为避免覆盖损坏已跳过入库，请先在模型页排查';
              }
            }
            if (!syncWarning) {
              const existing = new Set(poolModels.map((entry) => entry?.slug).filter(Boolean));
              models.forEach((m, index) => {
                const slug = String(m?.name || '').trim();
                if (!slug || existing.has(slug)) return;
                fresh.push({
                  slug,
                  display_name: String(m?.displayName || m?.name || slug),
                  ...(account.provider === 'openai'
                    ? { context_window: 272000, max_context_window: 272000, priority: 1 + index }
                    : { priority: 20 + index }),
                });
              });
              if (fresh.length) {
                const { pool: mergedPool } = ensureOfficialCatalogEntries(
                  dedupeCatalogEntries([...poolModels, ...fresh]),
                );
                const nextCatalog = buildModelsJson(mergedPool);
                if (!catalogRevision) {
                  // 池文件尚不存在：prepareJsonWrite 自带「目标文件写入期间被并发创建」
                  // 的冲突检测（O_EXCL 临时文件 + 身份复核 + rename 前比对）
                  const prepared = prepareJsonWrite(options.catalogPath, nextCatalog, {});
                  prepared.sync();
                  prepared.replace();
                } else {
                  commitRevisionedJson(options.catalogPath, nextCatalog, catalogRevision, {});
                }
                addedSlugs = fresh.map((entry) => entry.slug);
                options.log?.({ event: 'accounts.models_synced', provider: account.provider, added: addedSlugs.length, slugs: addedSlugs });
              }
            }
          }
        } catch (syncError) {
          // 落库失败不阻塞回显（上游清单仍返回给前端展示）；下次拉取会重试追加
          syncWarning = `入库失败：${String(syncError?.message || syncError).slice(0, 160)}`;
          options.log?.({ event: 'accounts.models_sync_failed', message: String(syncError?.message || syncError).slice(0, 200) });
        }
        jsonResponse(res, 200, { ok: true, provider: account.provider, source, count: models.length, models, addedSlugs, ...(syncWarning ? { syncWarning } : {}) });
      } catch (err) {
    ...
    if (req.method === 'POST' && url === '/_admin/api/accounts/test-model') {
      ...
      try {
        ...
        if (provider === 'chatgpt-web') {
          // 网页会话通道：真实 sentinel 门禁（bootstrap→PoW→prepare/finalize）+ 最小
          // conversation 请求；turnstile/arkose 风控信息原样透传给面板（实验通道最常见
          // 的不可用原因就是它），账号凭据由 getValidCredentials 统一临期刷新
          if (!model) {
            jsonResponse(res, 400, { error: { code: 'invalid_model', message: '缺少模型名' } });
            return true;
          }
          const result = await testChatGptWebModel({
            accessToken: creds.accessToken,
            model,
            proxy,
          });
    ...
    if (req.method === 'GET' && url.startsWith('/_admin/api/accounts/quota')) {
      ...
      if (account.provider === 'openai') {
        ...
        // 默认（无 force）回已有快照——首屏渲染不能被 45s 探测拖住；
        // force=1（面板「刷新」按钮）跳过缓存强制真探测，拿到上游当下真实额度。
        // 探测节流：同一账号 60s 内不重复打上游（防连点烧配额）；query 复用外层解析。
        const force = query.get('force') === '1';
        const rl = account.metadata?.rateLimits;
        const lastProbedAt = Number(rl?.updatedAt) || 0;
        if (!force && (rl?.fiveHour || rl?.weekly)) {
          jsonResponse(res, 200, {
            ok: true,
            provider: 'openai',
        ...
        if (force && Date.now() - lastProbedAt < 60_000) {
          jsonResponse(res, 429, {
            error: {
              code: 'quota_probe_throttled',
              message: `刚刚更新过（${new Date(lastProbedAt).toLocaleTimeString()}），1 分钟内无需重复探测`,
            },
          });
          return true;
        }
        try {
          ...
          const now = Date.now();
          authManager.updateAccount(account.id, { metadata: { ...(account.metadata || {}), rateLimits: { ...limits, updatedAt: now } } });
          // 快照落库：只写内存的话重启即丢（W1 被动学习同样受益于此通道）
          authManager.persistAccount?.(account.id);
          jsonResponse(res, 200, {
            ok: true,
            provider: 'openai',
    ...
    if (req.method === 'POST' && url === '/_admin/api/accounts/set-proxy') {
      ...
      // 落库：否则重启后账号代理静默回退 db 旧值（出站 IP 突变 / token 刷新走错链路）
      authManager.persistAccount?.(account.id);
      jsonResponse(res, 200, {
        ok: true,
        id: account.id,
    ...
    if (req.method === 'POST' && url === '/_admin/api/accounts/set-priority') {
      ...
      // 落库：metadata.priority 参与选号排序，重启不丢（与 set-proxy 同类修复）
      authManager.persistAccount?.(account.id);
      jsonResponse(res, 200, {
        ok: true,
        id: account.id,
    ...
    if (req.method === 'POST' && url === '/_admin/api/targets/fetch-models') {
      ...
      if (!apiKey) {
        // 与路由运行时同口径：区分「池空」「全冷却」「env_ref 解析失败」，不再一律报环境变量未设置。
        const shortfall = typeof keyPool?.describeKeyShortfall === 'function'
          ? keyPool.describeKeyShortfall(target)
          : { reason: 'empty' };
        const message = shortfall.reason === 'all_cooling'
          ? `通道 ${targetName} 的 ${shortfall.totalKeys ?? '全部'} 把密钥都在冷却中，请等冷却恢复或添加新 key 后再拉取`
          : shortfall.reason === 'env_ref_unresolved'
            ? `通道 ${targetName} 的密钥引用的环境变量未设置，请先在管理页修正后再拉取`
            : `通道 ${targetName} 没有配置任何可用密钥（密钥池为空${target.envKey ? `且环境变量 ${target.envKey} 未设置` : ''}），请先添加 key 后再拉取`;
        jsonResponse(res, 400, {
          error: {
    ...
    if (req.method === 'POST' && url === '/_admin/api/models/prefix-platform') {
      // 模型显示名加/去/重算平台前缀（Codex 下拉区分平台，如 opencode/deepseek-v4-pro）。
      // 前缀 = 模型路由目标短名（target.name 第一个 '-' 前段：opencode-go-chat → opencode）。
      const payload = await readJsonBody(req);
      const mode = payload?.mode === 'remove' ? 'remove'
        : payload?.mode === 'rebuild' ? 'rebuild'
        : 'add';
      const current = readModelRoutingFiles();
      const exposed = exposeModelRoutingState(
        current.catalog.value,
      ...
      // add/rebuild 模式跳过官方目标；remove 模式处理全部模型（含历史误加前缀的官方模型）。
      ...
      for (const model of models) {
        ...
        if (mode === 'rebuild') {
          // 重算：前缀一律改为当前路由通道的厂商短名，剥掉历史残留旧前缀
          // （zhipu-flash-plan 曾被冠以 b.ai/ 但实际走智谱订阅通道）；无路由模型不动。
          if (!prefix) continue;
          const dn = model.display_name;
          const slash = dn.indexOf('/');
          const base = (slash >= 0 ? dn.slice(slash + 1) : dn).trim();
          const rebuilt = `${prefix}/${base || model.slug}`;
          if (rebuilt !== dn) {
            model.display_name = rebuilt;
            changed += 1;
          }
        } else if (mode === 'add' && !model.display_name.startsWith(`${prefix}/`)) {
          if (!prefix) continue;
          model.display_name = `${prefix}/${model.display_name || model.slug}`;
          changed += 1;
        } else if (mode === 'remove' && prefix && model.display_name.startsWith(`${prefix}/`)) {
          model.display_name = model.display_name.slice(prefix.length + 1);
          changed += 1;
        }
      }
      if (changed === 0) {
        jsonResponse(res, 200, { ok: true, changed: 0, restartRequired: true, message: '无需变更（前缀已一致或无可匹配目标）' });
        return true;
      }
    ...
    // 面板产物新鲜度：web/ 产物随仓库分发，自更新本身就会带上新面板；这里校验的
    // 是发版侧「改了面板源码忘重新构建」的状态——产物内容指纹与当前源码不一致时
    // 面板顶部显式警告，避免后端新、面板旧且版本号还显示新版的无感知错位。
    if (req.method === 'GET' && url === '/_admin/api/panel-build') {
      const webRoot = options.webRoot || null;
      const runDir = webRoot ? path.dirname(webRoot) : null;
      const webAdminDir = runDir ? path.join(runDir, 'web-admin') : null;
      let sourceVersion = '';
      try {
        sourceVersion = JSON.parse(fs.readFileSync(path.join(runDir, 'package.json'), 'utf8')).version || '';
      } catch { /* 版本仅作展示 */ }
      const status = panelBuildStatus({ webRoot, webAdminDir });
      jsonResponse(res, 200, { ...status, sourceVersion });
      return true;
    }
    ...
    // 更新进度轮询（一键更新转圈无进度 → 阶段百分比实时可见）
    if (req.method === 'GET' && url === '/_admin/api/update/progress') {
      if (!selfUpdate) {
        jsonResponse(res, 400, { error: { code: 'update_unsupported', message: '当前部署形态不支持自更新' } });
        return true;
      }
      jsonResponse(res, 200, { ok: true, ...selfUpdate.progressSnapshot() });
      return true;
    }
    // 服务端强制压缩阈值（chatGuard）：对所有 chat 直连智能体生效的兜底裁剪线。
    // 读写 config.chatGuard；maxContextTokens=null 恢复默认 40 万。
    if (req.method === 'GET' && url === '/_admin/api/chat-guard') {
      const current = readAdminConfig(options);
      const guard = current.value?.chatGuard || {};
      jsonResponse(res, 200, {
        ok: true,
        enabled: guard.enabled !== false,
        maxContextTokens: Number(guard.maxContextTokens) > 0 ? Number(guard.maxContextTokens) : 400_000,
        defaultMaxContextTokens: 400_000,
      });
      return true;
    }
    if (req.method === 'POST' && url === '/_admin/api/chat-guard') {
      const payload = await readJsonBody(req);
      const enabled = payload?.enabled !== false;
      const current = readAdminConfig(options);
      let value;
      const raw = payload?.maxContextTokens;
      if (raw === undefined || raw === null || raw === '') {
        // 只切开关不带阈值：沿用现有配置（新装环境回落默认 40 万），不再误报 400
        const existing = Number(current.value?.chatGuard?.maxContextTokens);
        value = Number.isFinite(existing) && existing >= 10_000 && existing <= 8_000_000
          ? Math.round(existing)
          : 400_000;
      } else {
        value = Math.round(Number(raw));
        if (!Number.isFinite(value) || value < 10_000 || value > 8_000_000) {
          jsonResponse(res, 400, { error: { code: 'invalid_guard_tokens', message: '压缩阈值需在 1 万 ~ 800 万 tokens 之间' } });
          return true;
        }
      }
      const config = structuredClone(current.value);
      const guard = config.chatGuard || (config.chatGuard = {});
      guard.enabled = enabled;
      guard.maxContextTokens = value;
      const inspected = inspectRouterConfig(config, context());
      if (inspected.errors.length) {
        jsonResponse(res, 422, { error: { code: 'config_invalid', message: `配置校验失败：${inspected.errors[0]?.message || '未知错误'}` } });
        return true;
      }
      const committed = commitAdminConfig(options, config, current.revision);
      jsonResponse(res, 200, { ok: true, enabled, maxContextTokens: value, revision: committed.revision, restartRequired: true });
      return true;
    }
    ...
    if (req.method === 'POST' && url === '/_admin/api/service/restart') {
      import('node:child_process').then(({ spawn }) => {
        ...
        const command = process.platform === 'win32' ? 'powershell.exe' : 'bash';
        try {
          // 双脚本并存时只跑第一个命中的：两个都 spawn 会双重重启（二次重启
          // 会打断第一次启动的排空窗口）
...
async function stopListenersOnPorts(ports) {
  ...
  return stopped;
    ...
    if (req.method === 'GET' && url === '/_admin/api/requests') {
      // url 已被外层剥掉查询串：limit 必须从原始 req.url 取（否则恒为默认 50）
      let limit = 50;
      try {
        const parsed = Number(new URL(req.url || url, 'http://127.0.0.1').searchParams.get('limit'));
        if (Number.isFinite(parsed)) limit = Math.max(1, Math.min(500, Math.floor(parsed)));
      } catch { /* 无查询串或非法值按默认 */ }
    ...
    if (req.method === 'GET' && requestLogMatch) {
      let requestId = null;
      try {
        requestId = decodeURIComponent(requestLogMatch[1]);
      } catch {
        jsonResponse(res, 400, { error: { code: 'invalid_request_id', message: '日志 ID 编码非法' } });
        return true;
      }
      const entry = options.requestLog ? options.requestLog.get(requestId) : null;
      if (!entry) {
    ...
    if (req.method === 'GET' && url === '/_admin/api/codex-desktop/state') {
      ...
      // 官方条目在池中缺失时（全新机器，桌面端从未写过目录）用内置近似条目补齐，
      // 否则接入弹窗里官方模型整体消失，用户无从勾选。
    ...
    // ---------- 桌面端 MCP 插件管理（Codex 原生 [mcp_servers.*] 段） ----------
    // 官方插件机制 = config.toml 段声明，桌面进程自己拉起执行（含电脑操作类插件）。
    // 面板此前只能手工编辑 config.toml；这组端点把增删改测纳入管理面。
    // 写端点已纳入 codexDesktopMutex（见 handleApi 外层路由）。
    if (req.method === 'GET' && url === '/_admin/api/desktop/mcp-servers') {
      const configPath = codexConfigTomlPath();
      if (!configPath || !fs.existsSync(configPath)) {
        jsonResponse(res, 200, { ok: true, servers: [], configPath: configPath || '' });
        return true;
      }
      let servers = [];
      try {
        servers = listMcpServerSections(fs.readFileSync(configPath, 'utf8'));
      } catch (error) {
        jsonResponse(res, 500, { error: { code: 'mcp_parse_failed', message: `解析 config.toml 失败：${String(error?.message || error).slice(0, 200)}` } });
        return true;
      }
      jsonResponse(res, 200, { ok: true, servers, configPath });
      return true;
    }
    if (req.method === 'POST' && url === '/_admin/api/desktop/mcp-servers/upsert') {
      const payload = await readJsonBody(req);
      const configPath = codexConfigTomlPath();
      if (!configPath || !fs.existsSync(configPath)) {
        jsonResponse(res, 400, { error: { code: 'desktop_config_missing', message: '未找到 Codex 桌面端 config.toml' } });
        return true;
      }
      const server = {
        ...
        args: Array.isArray(payload?.args)
          ? payload.args.filter((item) => typeof item === 'string').map((item) => item.trim()).filter(Boolean)
          : [],
        startupTimeoutSec: Number.isFinite(Number(payload?.startupTimeoutSec)) && Number(payload.startupTimeoutSec) >= 1
          ? Math.floor(Number(payload.startupTimeoutSec))
          : null,
        toolTimeoutSec: Number.isFinite(Number(payload?.toolTimeoutSec)) && Number(payload.toolTimeoutSec) >= 1
          ? Math.floor(Number(payload.toolTimeoutSec))
          : null,
        enabled: payload?.enabled !== false,
        enabledTools: Array.isArray(payload?.enabledTools)
          ? payload.enabledTools.filter((item) => typeof item === 'string' && item.trim()).map((item) => item.trim()).slice(0, 64)
          : null,
        disabledTools: Array.isArray(payload?.disabledTools)
          ? payload.disabledTools.filter((item) => typeof item === 'string' && item.trim()).map((item) => item.trim()).slice(0, 64)
          : null,
        env: payload?.env && typeof payload?.env === 'object' && !Array.isArray(payload?.env)
          ? Object.fromEntries(Object.entries(payload.env).map(([k, v]) => [String(k), String(v)]))
          : {},
      };
      const original = listMcpServerSections(fs.readFileSync(configPath, 'utf8'))
        .find((entry) => entry.name === server.name);
      if (original && !original.editable) {
        jsonResponse(res, 400, {
          error: {
            code: 'mcp_section_not_editable',
            message: `该段含面板不认识的高级配置（${(original.extraKeys || []).join(', ') || '非标准形态'}），为防丢字段请手工编辑 config.toml`,
          },
        });
        return true;
      }
      try {
        const next = upsertMcpServerSection(fs.readFileSync(configPath, 'utf8'), server);
        const backup = writeWithBackup(configPath, next);
        options.log?.({ event: 'desktop.mcp_server_upserted', name: server.name });
        jsonResponse(res, 200, {
          ok: true,
          name: server.name,
          backup,
          restartRequired: true,
          message: `已保存插件 ${server.name}；需重启桌面端后生效（可用上方「重启 ChatGPT 桌面端」按钮）`,
        });
      } catch (error) {
        jsonResponse(res, 400, { error: { code: 'mcp_upsert_failed', message: String(error?.message || error).slice(0, 300) } });
      }
      return true;
    }
    if (req.method === 'POST' && url === '/_admin/api/desktop/mcp-servers/delete') {
      const payload = await readJsonBody(req);
      const configPath = codexConfigTomlPath();
      const name = typeof payload?.name === 'string' ? payload.name.trim() : '';
      if (!configPath || !fs.existsSync(configPath) || !name) {
        jsonResponse(res, 400, { error: { code: 'invalid_request', message: '缺少插件名或 config.toml 不可用' } });
        return true;
      }
      const content = fs.readFileSync(configPath, 'utf8');
      const original = listMcpServerSections(content).find((entry) => entry.name === name);
      if (!original) {
        jsonResponse(res, 404, { error: { code: 'mcp_server_not_found', message: `未找到插件段 [mcp_servers.${name}]` } });
        return true;
      }
      if (!original.editable) {
        jsonResponse(res, 400, {
          error: { code: 'mcp_section_not_editable', message: '该段含面板不认识的高级配置，为防丢字段请手工编辑 config.toml' },
        });
        return true;
      }
      try {
        const backup = writeWithBackup(configPath, removeMcpServerSection(content, name));
        options.log?.({ event: 'desktop.mcp_server_removed', name });
        jsonResponse(res, 200, { ok: true, backup, restartRequired: true, message: `已移除插件 ${name}；重启桌面端后生效` });
      } catch (error) {
        jsonResponse(res, 400, { error: { code: 'mcp_delete_failed', message: String(error?.message || error).slice(0, 300) } });
      }
      return true;
    }
    if (req.method === 'POST' && url === '/_admin/api/desktop/mcp-servers/test') {
      const payload = await readJsonBody(req);
      // 支持两种入参：完整定义（新增前先测）或按名取既有段
      let server = null;
      if (typeof payload?.name === 'string' && !payload?.command) {
        const configPath = codexConfigTomlPath();
        const content = configPath && fs.existsSync(configPath) ? fs.readFileSync(configPath, 'utf8') : '';
        server = listMcpServerSections(content).find((entry) => entry.name === payload.name) || null;
        if (!server) {
          jsonResponse(res, 404, { error: { code: 'mcp_server_not_found', message: `未找到插件段 [mcp_servers.${payload.name}]` } });
          return true;
        }
      } else {
        server = {
      ...
      if (!server.command) {
        jsonResponse(res, 400, { error: { code: 'invalid_request', message: '缺少 command' } });
        return true;
      }
      const timeoutMs = Math.min(60_000, Math.max(5_000, Number(payload?.timeoutMs) || 20_000));
    ...
    if (req.method === 'POST' && url === '/_admin/api/codex-desktop/restore-official') {
      const payload = await readJsonBody(req);
      // 默认模型必须是官方存量 slug：官方直连下网页池/第三方模型（web-gpt-5-6 等）
      // 在 OpenAI 侧不存在，写进 config.toml 会让桌面端默认模型直接报错
      // （2026-09-10 实锤：恢复官方直连后 model 仍是 web-gpt-5-6）
      const requestedModel = typeof payload?.defaultModel === 'string' ? payload.defaultModel.trim() : '';
      const defaultModel = OFFICIAL_MODEL_SLUGS.includes(requestedModel) ? requestedModel : 'gpt-5.6-sol';
      ...
      // 历史会话线程迁移：路由模式下创建的线程钉在 provider=router 上（线程级绑定），
      // 不迁移的话旧会话（如 /goal 长任务）恢复官方直连后继续走路由——2026-09-10
      // 用户实锤「重启了还不是官方直连」。全部迁回 openai（官方直连），走账号自带额度池。
      let sessionSyncNote = '';
      let migratedSessions = 0;
      try {
        const { syncSessionProviders } = await import('./codex-session-sync.mjs');
        const sync = syncSessionProviders({ targetProvider: 'openai', enabledProviders: ['openai', 'router'] });
        migratedSessions = sync.sqliteThreads + sync.rolloutFiles;
        if (migratedSessions > 0) {
          sessionSyncNote = `；已把 ${sync.sqliteThreads} 个历史会话线程迁回官方直连`;
        }
      } catch { /* 迁移失败不影响恢复 */ }
      jsonResponse(res, 200, {
        ok: true,
        mode: 'official',
        backupToml,
        message: '已恢复官方直连配置（保留全部现有配置与模型目录）'
          + (sessionSyncNote || '')
          + '；请完全退出并重启 Codex 桌面端生效',
      });
      return true;
    }
    // ---------- 订阅账号上游模型自动进池 ----------
    // 「拉取上游可用模型」的结果此前只在订阅页回显，不落模型池：全新机器接入路由时
    // 官方模型整体从桌面选择器消失（2026-09-06 macOS 首装实测实锤）。apply-router
    // 前对每个绑定账号（ChatGPT / Claude / 谷歌）拉取上游清单并转成目录条目；
    // 单账号失败不阻塞（openai 回退内置清单），订阅页手动按钮仍是主动刷新入口。
    async function collectSubscriptionUpstreamEntries() {
      const authManager = options.authManager;
      if (!authManager || typeof authManager.listAccounts !== 'function') {
        return { entries: [], openaiSlugs: new Set(), errors: [] };
      }
      const accounts = authManager.listAccounts().filter((a) => a && a.id && a.provider);
      if (!accounts.length) return { entries: [], openaiSlugs: new Set(), errors: [] };
      const proxy = options.oauthProxy || options.proxy;
      const entries = [];
      const openaiSlugs = new Set();
      const errors = [];
      await Promise.all(accounts.map(async (account) => {
        try {
          ...
          if (account.provider === 'openai') {
            try {
              models = await fetchOpenAiCodexModels({ accessToken: credentials.accessToken, proxy });
            } catch {
              models = CODEX_KNOWN_MODELS.map((m) => ({ ...m }));
            }
          } else if (account.provider === 'claude') {
            ...
            const projectId = credentials.projectId || account.metadata?.projectId || '';
            models = await fetchGoogleAvailableModels({ accessToken: credentials.accessToken, projectId, proxy });
          } else {
            return;
          }
          models.forEach((m, index) => {
            ...
            if (!slug) return;
            if (account.provider === 'openai') {
              openaiSlugs.add(slug);
              // 拉取清单里混着 API 形态的 dashed 变体（gpt-5-5 / gpt-5-6-instant /
              // gpt-6-pro…），与账号原生的 dotted slug（gpt-5.5 / gpt-5.6-sol）是同一
              // 批模型的两种写法，全部进池会在桌面选择器/面板里刷出成排重复（2026-09-10
              // 用户实锤）。原生 dotted 形态才是桌面端一等公民，dashed 变体不再入池。
              if (/^gpt-\d+-\d/.test(slug) || slug === 'gpt-6-pro') return;
            }
            entries.push({
              slug,
              display_name: String(m?.displayName || m?.name || slug),
          ...
          errors.push(`${account.provider}: ${error?.message || error}`);
        }
      }));
      return { entries, openaiSlugs, errors };
    }
    ...
    if (req.method === 'POST' && url === '/_admin/api/codex-desktop/apply-router') {
      ...
      const poolModels = readPoolCatalog();
      const loadedModels = readDesktopLoadedCatalog();
      // 绑定订阅账号（ChatGPT / Claude / 谷歌）自动拉取上游可用模型进池；
      // 仍缺失的官方 slug 再用内置近似条目兜底（无绑定账号/拉取全失败的全新机器）。
      const upstream = await collectSubscriptionUpstreamEntries();
      const { pool: officialAwarePool, synthesizedSlugs } = ensureOfficialCatalogEntries(
        dedupeCatalogEntries([...poolModels, ...loadedModels, ...upstream.entries]),
      );
      const upstreamOfficialSlugs = upstream.openaiSlugs;
      ...
      // 官方集合 = 内置 slug 表 ∪ ChatGPT 账号实时拉取的 slug（上游新增模型如
      // gpt-6-astra 未进内置表时也按官方常驻，不会在下次接入时被挤掉）。
      const official = catalogPool.filter((entry) => (
        OFFICIAL_MODEL_SLUGS.includes(entry.slug) || upstreamOfficialSlugs.has(entry.slug)
      ));
      ...
      if (apiKeyMode) {
        // 先复用 config.toml 里已有的路由 token（后缀能对上未吊销 key 才算有效）：
        // 每次 apply 都铸新 key 会让 desktop-apikey 无限堆积（2026-09-09 实测积了 7 把）。
        // 只认 [model_providers.router] 段内的同名字段——其他自定义 provider 段
        // 也可能用 experimental_bearer_token，抓全文件第一处会拿到异构 token。
        const existingToken = (() => {
          const toml = String(currentToml || '');
          const section = toml.indexOf('[model_providers.router]');
          if (section < 0) return '';
          const scope = toml.slice(section);
          const nextSection = scope.indexOf('\n[');
          return /experimental_bearer_token\s*=\s*"([^"]+)"/
            .exec(nextSection > 0 ? scope.slice(0, nextSection) : scope)?.[1] || '';
        })();
        if (existingToken && apiKeyStore.verifyKey(existingToken)) {
          apiBearerToken = existingToken;
          reusedToken = true;
        } else {
          try {
      ...
      try {
        ...
        // 上游拉取到的订阅模型持久化进模型池（models.json）：官方/订阅条目从此在
        // 分组模型页与接入弹窗可见，下次接入即使拉取失败也有存货可回填。
        // 池内已有条目一律保留原值，只追加新 slug。
        if (upstream.entries.length) {
          const existingSlugs = new Set(readPoolCatalog().map((entry) => entry?.slug).filter(Boolean));
          const freshUpstream = upstream.entries.filter((entry) => !existingSlugs.has(entry.slug));
          if (freshUpstream.length) {
            writeWithBackup(
              desktopPaths.modelsJson,
              buildModelsJson(dedupeCatalogEntries([...readPoolCatalog(), ...freshUpstream])),
            );
          }
        }
      } catch (error) {
      ...
      jsonResponse(res, 200, {
        ...
        message: (apiKeyMode && apiBearerToken
          ? `已接入路由（API-key 形态：官方额度耗尽也能用自定义模型；桌面端将识别为 LocalRouter，不再显示官方额度条）。桌面端专用 API Key：${apiBearerToken.slice(0, 10)}...（${reusedToken ? '复用现有密钥' : '本次新增'}，可在「API 密钥管理」吊销）；完全退出重启桌面端生效`
          : `已接入路由（${merged.length} 个模型，默认 ${defaultModel}）；请完全退出并重启 Codex 桌面端生效`)
          + (upstream.entries.length
            ? `；已自动拉取 ${upstream.entries.length} 个绑定账号的上游可用模型进池`
            : '')
          + (upstream.errors.length
            ? `（部分账号拉取失败：${upstream.errors.join('；')}）`
            : '')
          + (synthesizedSlugs.length
            ? `；模型池缺少官方条目，已按内置近似元数据补齐 ${synthesizedSlugs.length} 个官方模型（桌面端在官方模式写过目录后会以真实值覆盖）`
            : '')
          + sessionSyncNote,
      });
      return true;
    ...
    if (url.startsWith('/_admin/api/cursor-gateway')) {
      ...
      try {
        ...
        // 停止网关：不使用时释放硬件资源（侧车 + Cursor SDK sidecar 都挂在 6718/6719 上）。
        // 账号池与凭据由网关自身持久化保留，下次启动自动加载。
        if (req.method === 'POST' && action === 'stop') {
          ...
          jsonResponse(res, 200, {
            ...
            message: stopped.length
              ? `网关已停止（端口 ${gatewayPort}/${gatewayPort + 1} 进程已退出，资源已释放）；账号池与凭据保留，下次启动自动加载`
              : '网关本就未在运行',
          });
          return true;
        }
        ...
        // 首次启动自动装依赖：external/cursor2api 的 node_modules 被 gitignore，新 clone 机器上
        // 缺失，直接后台拉起会静默失败（报错被 stdio ignore 吞掉，面板只见 ECONNREFUSED）。
        // bun 检测与自动安装：网关的 Cursor SDK sidecar 用 `bun run` 拉起（server.mjs
        // assertDependencies 硬性要求 PATH 有 bun），全新机器没有 bun 时启动必败——
        // 且报错被吞后只能看到 ECONNREFUSED（2026-09-06 macOS 首装实测）。
        // 走 npm 全局安装跨平台且免 sudo（npm 全局 bin 与 node 同目录，已在 PATH）。
        async function ensureBunAvailable() {
          const { execFile } = await import('node:child_process');
          const bunVersion = () => new Promise((resolve, reject) => {
            execFile('bun', ['--version'], { timeout: 30_000, windowsHide: true, shell: true }, (err, stdout) => {
              if (err) { reject(err); return; }
              resolve(String(stdout).trim());
            });
          });
          try {
            return { installed: false, version: await bunVersion() };
          } catch { /* 未安装：继续自动安装 */ }
          await new Promise((resolve, reject) => {
            execFile('npm', ['install', '-g', 'bun', '--no-audit', '--no-fund', '--loglevel=error'], {
              timeout: 300_000, maxBuffer: 8 * 1024 * 1024, windowsHide: true, shell: true,
            }, (err, stdout, stderr) => {
              if (err) {
          ...
          try {
            return { installed: true, version: await bunVersion() };
          } catch {
            throw new Error('Cursor 网关依赖 bun（Cursor SDK sidecar 运行时），自动安装后仍未在 PATH 找到 bun。请手动安装后重试：npm install -g bun，或 https://bun.sh');
          }
        }
        async function ensureCursorGatewayDeps(dir) {
          if (fs.existsSync(path.join(dir, 'node_modules', '@cursor', 'sdk'))) {
            return { installed: false };
          }
          const { execFile } = await import('node:child_process');
          const runNpmInstall = (extraArgs) => new Promise((resolve, reject) => {
            // shell:true 兼容 Windows 的 npm.cmd；参数全是固定字面量，无注入面
            execFile('npm', ['install', ...extraArgs, '--no-audit', '--no-fund', '--loglevel=error'], {
              cwd: dir, timeout: 300_000, maxBuffer: 8 * 1024 * 1024, windowsHide: true, shell: true,
            }, (err, stdout, stderr) => {
              if (err) {
          ...
          try {
            ...
            // 网关仓库未提交 lockfile，依赖区间漂浮：上游 peer 升级（实测 wrangler 要
            // workers-types@5、根项目锁 @4）会让严格解析 ERESOLVE 失败。按 npm 官方
            // 建议降级 --legacy-peer-deps 重试；冲突的仅是部署工具链，不影响网关运行。
            lastError = String(firstError?.message || firstError);
            await runNpmInstall(['--legacy-peer-deps']);
          }
          if (!fs.existsSync(path.join(dir, 'node_modules', '@cursor', 'sdk'))) {
            throw new Error(`网关依赖安装后仍缺少 @cursor/sdk${lastError ? `（首次严格安装报错：${lastError}）` : ''}，请到 external/cursor2api 手动执行 npm install 查看具体报错`);
          }
        ...
        if (req.method === 'POST' && (action === 'restart' || action === 'start')) {
          try {
            ...
            const deps = await ensureCursorGatewayDeps(dir);
            const bun = await ensureBunAvailable();
            // 管理密码自动配对：网关控制台密码来自 ADMIN_PASSWORD，路由代理的管理会话
            // 用 CURSOR_GATEWAY_ADMIN_PASSWORD 登录。全新机器两者都没配 → 网关在跑但
            // 面板账号管理 401/422（2026-09-06 macOS 首装实测）。自动生成随机密码同步
            // 注入两侧；密码持久化到 data/（0600，不入库）跨路由重启复用——网关侧
            // auth-state.json 的哈希只在首次播种，路由侧换密码必然 401。用户显式设置
            // 的环境变量始终优先。
            let provisionedNow = false;
            if (!process.env.CURSOR_GATEWAY_ADMIN_PASSWORD) {
              const pwFile = path.join(path.dirname(options.configPath || process.cwd()), 'data', 'cursor-gateway.admin-password');
              try {
                if (fs.existsSync(pwFile)) {
                  process.env.CURSOR_GATEWAY_ADMIN_PASSWORD = fs.readFileSync(pwFile, 'utf8').trim();
                }
              } catch { /* 读不到走生成 */ }
              if (!process.env.CURSOR_GATEWAY_ADMIN_PASSWORD) {
                process.env.CURSOR_GATEWAY_ADMIN_PASSWORD = crypto.randomBytes(16).toString('hex');
                provisionedNow = true;
                try {
                  fs.mkdirSync(path.dirname(pwFile), { recursive: true });
                  fs.writeFileSync(pwFile, process.env.CURSOR_GATEWAY_ADMIN_PASSWORD, { mode: 0o600 });
                } catch { /* 持久化失败则仅本次会话内可用 */ }
              }
              gatewayCookieJar.cookie = '';
              // 迁移自愈：auth-state.json 先于路由托管存在（其密码无人知晓）时，env 播种
              // 会被跳过、登录必 401。改名备份后由网关按我们的密码重新播种。
              const gatewayAuthState = path.join(os.homedir(), '.cursor2api', 'auth-state.json');
              if (provisionedNow && fs.existsSync(gatewayAuthState)) {
                try { fs.renameSync(gatewayAuthState, `${gatewayAuthState}.bak-${Date.now()}`); } catch { /* 保留原状态则登录失败会明确报错 */ }
              }
            }
            const stopped = await stopListenersOnPorts([gatewayPort, gatewayPort + 1]);
            const { spawn } = await import('node:child_process');
            // 启动报错落盘（此前 stdio ignore 吞错，排障只能看到 ECONNREFUSED）
            let stdioSpec = 'ignore';
            try {
              const logFd = fs.openSync(path.join(dir, 'gateway.start.log'), 'a');
              stdioSpec = ['ignore', logFd, logFd];
            } catch { /* 日志文件打不开则维持 ignore */ }
            const child = spawn(
              process.execPath,
              ['server.mjs', 'start', '--port', String(gatewayPort)],
              {
                ...
                env: {
                  ...
                  // 控制台管理密码与路由代理登录用同一把，保证面板开箱即用
                  ADMIN_PASSWORD: process.env.CURSOR_GATEWAY_ADMIN_PASSWORD || '',
                },
              },
            );
            child.unref();
            jsonResponse(res, 200, {
              ...
              message: (deps.installed ? '首次启动已自动安装网关依赖（npm install）；' : '')
                + (bun.installed ? `已自动安装 bun ${bun.version}（Cursor SDK sidecar 运行时）；` : '')
                + (action === 'start' && stopped.length === 0
  ...
  return async function adminHandler(req, res) {
    ...
    try {
      ...
      // mcp-servers 写入多文件读-改-写窗口内串行化，防并发丢更新（switch-account 另有自身互斥）
      const desktopWriteLock = url === '/_admin/api/codex-desktop/restore-official'
        ...
        || (url === '/_admin/api/desktop/mcp-servers/upsert' && req.method === 'POST')
        || (url === '/_admin/api/desktop/mcp-servers/delete' && req.method === 'POST');
      if (desktopWriteLock) {
