      ...
      try {
        ...
        try {
          if (Array.isArray(models) && models.length && options.catalogPath) {
            ...
            let poolModels = [];
            try {
              const poolData = JSON.parse(fs.readFileSync(options.catalogPath, 'utf8'));
              if (Array.isArray(poolData?.models)) poolModels = poolData.models;
            } catch { /* 池文件缺失/损坏按空池处理，只追加不破坏 */ }
            ...
            if (fresh.length) {
              ...
              writeWithBackup(options.catalogPath, buildModelsJson(mergedPool));
        ...
        jsonResponse(res, 200, { ok: true, provider: account.provider, source, count: models.length, models, addedSlugs });
      } catch (err) {
