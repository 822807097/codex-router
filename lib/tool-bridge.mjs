...
export function readToolsConfig(config = {}) {
  ...
  return {
    ...
    // 工具循环轮次上限：多步 MCP/电脑操作任务 3 轮必断（2026-09-13 调研 B2.1），
    // 默认 8、可配 1..32；耗尽时在最终回答里显式标注而非无声截断
    maxBridgeRounds: Math.min(Math.max(Number(tools.maxBridgeRounds) || 8, 1), 32),
    // 网页池 CU 工具目录注入开关（默认开；CHATGPT_WEB_MCP_INJECT=0 逃生门优先）
    webPoolInject: tools.webPoolInject !== false,
...
class McpStdioClient {
  ...
  start() {
    ...
    // spawn 失败（command 不存在 ENOENT / Windows 裸 .cmd 名等）走 'error' 事件
    // 且不会触发 'exit'——不挂监听会升级成 uncaughtException 打穿进程熔断
    // （2026-09-13 对抗审查实锤：管理页「测试连接」按错一次即熔断计数）
    this.process.once('error', (error) => {
      this.process = null;
      for (const resolver of this.pending.values()) {
        resolver({ error: { code: -1, message: `MCP server 启动失败：${error?.message || error}` } });
      }
      this.pending.clear();
      this.tools = null;
    });
  ...
  /** 原始 tools/call：返回 MCP result（content 数组含 text/image 等），供门面透传。 */
...
/**
 * 一次性探测 MCP server（管理页「测试连接」用）：initialize + tools/list 后即退出，
 * 不进常驻缓存。返回 { ok, toolCount, tools, latencyMs, error }。
 */
export async function probeMcpServer(server, timeoutMs = 20_000) {
  const full = await probeMcpServerFull(server, timeoutMs);
  return {
    ok: full.ok,
    toolCount: full.tools.length,
    tools: full.tools.map((tool) => tool.name).slice(0, 50),
...
/**
 * 与 probeMcpServer 同链路但返回完整 schema（工具目录单一事实源拉取用，P1.3）。
 * 返回 { ok, tools:[{name,description,inputSchema}], latencyMs, error }。
 */
export async function probeMcpServerFull(server, timeoutMs = 20_000) {
  ...
  try {
    const tools = await Promise.race([
      ...
      new Promise((_, reject) => {
        timer = setTimeout(() => reject(new Error(`探测超时（>${Math.round(timeoutMs / 1000)}s，进程可能未就绪或启动很慢）`)), timeoutMs);
        timer.unref?.();
      }),
    ]);
    return {
    ...
    let message = String(error?.message || error).slice(0, 300);
    // Windows 常见坑提示：Node spawn 不走 shell，npx/npm 等 .cmd 垫片必须写完整路径
    if (/ENOENT/i.test(message) && !/[\\/]/.test(String(server?.command || ''))) {
      message += '（Windows 上 npx/npm 等 .cmd 命令需写完整路径，如 C:\\Program Files\\nodejs\\npx.cmd，或改用 node + 脚本路径）';
    }
    return { ok: false, tools: [], error: message, latencyMs: Date.now() - startedAt };
  } finally {
...
/**
 * 按 server 名执行 tools/call（常驻热客户端，供原生 MCP 门面执行面使用）。
 * server 未在 tools.mcpServers 配置或已禁用时抛错。
 */
export async function callMcpServerTool(toolsConfig, serverName, toolName, args) {
  const server = (toolsConfig?.mcpServers || []).find((s) => s.name === serverName && s.enabled);
  if (!server) throw new Error(`MCP server ${serverName} 未配置或已禁用`);
  return getMcpClient(server).callTool(toolName, args);
}

/**
 * 按 spec 直连执行（目录解析兜底路径，2026-09-14 P4.4）：
 * tools.mcpServers 未声明 windows_computer_use 时，路由从桌面 config.toml 解析出
 * 启动 spec——执行面必须与目录同源，否则门面工具目录在列但调用全部落空。
 */
export async function callMcpToolBySpec(spec, toolName, args) {
  if (!spec?.command) throw new Error('MCP server spec 未提供');
  const server = {
    name: `spec:${spec.command}|${(spec.args || []).join(' ')}`,
...
/** 按 server 名拉工具表（常驻热客户端，含空闲回收钩子）。 */
export async function listMcpServerTools(toolsConfig, serverName) {
  const server = (toolsConfig?.mcpServers || []).find((s) => s.name === serverName && s.enabled);
  if (!server) throw new Error(`MCP server ${serverName} 未配置或已禁用`);
  return getMcpClient(server).listTools();
}

/** 启用的技能拼接为 system 附加指令（Codex 未提供的技能由路由注入）。 */
export function buildSkillsSystemAppendix(toolsConfig) {  const enabled = toolsConfig.skills.filter((s) => s.enabled && s.content.trim());
