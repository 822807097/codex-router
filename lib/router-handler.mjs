          ...
          try {
            ...
            if (isChat) {
              ...
              if (isChatCompletionsFormat) {
                ...
                // 服务端滑动窗口：第三方智能体（ZCode 等）自己不做上下文压缩时，
                // 历史每轮全量重发会无限膨胀（实测 17.3MB body 上游网关直接
                // 「read body failed」400）。超预算（6MB）时裁掉最旧的完整轮次：
                // 保 system、绝不拆散 tool_calls 配对、保留最新轮次（trimToBudget
                // 语义），任务带着近期上下文继续而非硬死。
                if (attemptBody && Array.isArray(attemptBody.messages) && attemptBody.messages.length > 40) {
                  const messageBytes = attemptBody.messages.reduce(
                    (sum, m) => sum + Buffer.byteLength(JSON.stringify(m)), 0,
                  );
                  const MAX_CHAT_CONTEXT_BYTES = 6 * 1024 * 1024;
                  if (messageBytes > MAX_CHAT_CONTEXT_BYTES) {
                    const fitted = trimToBudget(
                      attemptBody.messages,
                      MAX_CHAT_CONTEXT_BYTES,
                      (m) => Buffer.byteLength(JSON.stringify(m)),
                    );
                    if (fitted.length < attemptBody.messages.length) {
                      flog({
                        ...
                        bytes_before: messageBytes,
                        bytes_after: fitted.reduce((sum, m) => sum + Buffer.byteLength(JSON.stringify(m)), 0),
                      });
