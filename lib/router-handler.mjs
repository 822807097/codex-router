...
import { openHttpsStream, rawHttpsRequest, resolveTimeouts } from './transport.mjs';
...
// 流自身异常不得顶替原失败：捕获后返回已读片段（保留原 status/quotaObservation）
...
// retry-after 头解析：重复头在 parseResponseHead 里保留为数组，取首个可用值
function retryAfterSeconds(headers) {
  const raw = headers?.['retry-after'];
  const value = Array.isArray(raw) ? raw[0] : raw;
  const seconds = Number(typeof value === 'string' ? value : NaN);
...
export function createRouterHandler(options) {
  ...
  // 官方通道增量续聊（省官方 5h 窗口；默认关闭——上游 2026-09-12 移除
  // previous_response_id 支持，显式开启 + 上游恢复时才生效）。运行时熔断在
  // 使用点生效（连续 3 次「参数不支持」类拒绝后本进程停用，重启重探）。
  let officialIncrementalRejectStreak = 0;
  let officialIncrementalDisabled = false;
  // 桌面端 OAuth 身份放行是常态路径：日志只记首次，避免每请求一条噪音
  let desktopOauthFallbackLogged = false;
  ...
  return async function routerHandler(clientReq, clientRes) {
    ...
    if (apiKeyStore && typeof apiKeyStore.hasKeys === 'function' && apiKeyStore.hasKeys()) {
      ...
      if (!verified && isLoopbackDesktopOauth(clientReq, authHeader, providedKey)) {
        ...
        // 桌面端走 OAuth 身份是常态路径（非异常 fallback）：只记本进程首次，
        // 之后静默——此前每请求一条把真异常信号淹没（2026-09-13 取证发现 55/55）
        if (!desktopOauthFallbackLogged) {
          desktopOauthFallbackLogged = true;
          flog({ event: 'auth.desktop_oauth_fallback', path: url, note: '桌面端 OAuth 身份为常态路径，本事件仅首次记录' });
        }
      } else if (!verified) {
    ...
    const finishDiagnostic = (clientStatus, disconnected = false) => {
      ...
      // 先收口诊断再取 summary：此前顺序相反，outcome 恒为 'running'，
      // 请求查看器里所有成功请求的 error 字段都显示 "running"（2026-09-13 审查）
    ...
    clientReq.on('end', async () => {
      ...
      try {
        ...
        for (let index = 0; index < compatibleCandidates.length; index += 1) {
          ...
          try {
            ...
            if (isChat) {
              ...
              if (isChatCompletionsFormat) {
                ...
                if (attemptBody && options.toolBridge) {
                  ...
                  if (bridgeActive && builtinTools.length) {
                    ...
                    // 桥接期间客户端可能断开：写回前必须检查（写已销毁响应会触发
                    // uncaughtException 计入熔断）；noop error 监听兜底异步 write 错误。
                    clientRes.once('error', () => { /* 客户端断开的写错误在此吸收 */ });
                    const callUpstream = async () => {
                      // 走 transport 统一出口：代理（target.proxyUrl/全局）与连接超时
                      // 与主路径一致；此前裸 fetch 不走代理、不接请求级 abort——
                      // viaProxy 通道直接无网，客户端断开后仍继续烧上游轮次。
                      const requestMs = resolveTimeouts(options.timeouts, provider.timeouts).requestMs || 180_000;
                      ...
                      let json = null;
                      try { json = JSON.parse(response.bodyText); } catch { json = {}; }
                      if (response.status < 200 || response.status >= 300) {
                        const failure = statusFailure(response.status, String(json?.error?.message || json?.error || `上游 ${response.status}`).slice(0, 300));
                    ...
                    // 轮次上限可配（config.json tools.maxBridgeRounds，默认 8）；
                    // 耗尽时向客户端显式标注，不再无声截断（2026-09-13 调研 B2.1）
                    const maxBridgeRounds = toolsCfg.maxBridgeRounds;
                    try {
                      let finalJson = await callUpstream();
                      let roundsExhausted = false;
                      for (let round = 0; round < maxBridgeRounds; round += 1) {
                        // 客户端已断开：停止后续上游轮次（不再烧额度），也无法再回写
                      ...
                      if (roundsExhausted) {
                        // 耗尽时模型消息往往是「还想调工具」的形态：content 为空、
                        // tool_calls 指向路由私有 builtin（客户端不认识无法执行）——
                        // 必须清掉并以标注作为正文，否则空响应 + 私有调用泄漏
                        //（2026-09-13 对抗审查：content 为空时标注被跳过、意图落空）
                        const notice = `（提示：已达工具调用轮次上限 ${maxBridgeRounds}，任务可能尚未完成——请继续对话接着执行。）`;
                        finalMessage.content = finalMessage.content
                          ? `${finalMessage.content}\n\n${notice}`
                          : notice;
                        finalMessage.tool_calls = undefined;
                        if (finalJson?.choices?.[0]) {
                          finalJson.choices[0].finish_reason = 'stop';
                        }
                      }
                ...
                const upstream = provider.platform === 'google'
                  ...
                  : provider.platform === 'chatgpt-web'
                    ? await openChatGptWebChatStream({
                      ...
                      timeouts,
                      log: flog,
                      webPoolMcpInject: readToolsConfig(options.config).webPoolInject,
                      nativeToolsConfig: (options.config.chatgptWeb || {}).nativeTools || null,
                    })
                ...
                requestDiagnostics.upstream({
                  ...
                  content_type: String(upstream.headers["content-type"] || "").slice(0, 60),
                });
                ...
                if (upstream.status !== 200) {
                  ...
                  if (retryAfter !== null) failure.retryAfter = String(retryAfter);
                  throw failure;
                }
              ...
              const upstreamPath = joinUpstreamPath(target.prefix, provider.chatPath);
              // 转换链路（responses→chat）的体积守卫：图片按固定 token 计费会漏掉
              // MB 级 data URL，透传链路有 8MB 快速失败而这里此前没有（2026-09-13
              // 审查 P2：大图请求绕过估算直达上游，报模糊 400）
              const preparedBytes = Buffer.byteLength(JSON.stringify(prepared.request));
              if (preparedBytes > 8 * 1024 * 1024) {
                requestDiagnostics.markFailure({
                  outcome: 'router_rejected',
                  error_code: 'session_payload_too_large',
                  error_stage: 'chat_request_prepared',
                });
                if (!clientRes.headersSent) {
                  clientRes.writeHead(413, { 'content-type': 'application/json' });
                }
                if (!clientRes.destroyed && !clientRes.writableEnded) {
                  clientRes.end(JSON.stringify({
                    error: {
                      code: 'session_payload_too_large',
                      message: `转换后的请求体 ${(preparedBytes / 1024 / 1024).toFixed(1)}MB 超过上游可靠处理上限（8MB）：会话历史/图片附件已过大。请新开会话，或让智能体清理大附件（截图/base64）后重试。`,
                    },
                  }));
                }
                return true;
            ...
            if (!isChat && isChatFormatRequest && provider.wireApi === 'responses') {
              ...
              if (upstream.status < 200 || upstream.status >= 300) {
                ...
                const bridgeRetryAfter = retryAfterSeconds(upstream.headers);
                if (bridgeRetryAfter !== null) {
                  failure.retryAfter = String(bridgeRetryAfter);
                }
                requestDiagnostics.markFailure({
            ...
            if (officialIncrementalStore && !officialIncrementalDisabled && /\/responses(?:\/|$|\?)/.test(officialIncUrl)
              && Array.isArray(attemptBody?.input)) {
              ...
              if (officialInc) {
                // 回退基线快照：必须在改写 attemptBody 之前留存原始
                // previous_response_id/input/store——增量被上游 4xx 拒绝时
                // 按此还原全量重发（否则会发出 delta-only 请求静默丢上下文）
                officialIncBase = {
                  previous_response_id: attemptBody.previous_response_id,
                  input: attemptBody.input,
                  store: attemptBody.store,
                };
                // 增量改写：上游按 previous_response_id 续算，本轮只发新增。
                // store 不再强设 true——官方 codex 后端 2026-09 起强制 store:false
                // （400 "Store must be set to false"），强设会让每轮增量必拒
              ...
              } else if (officialIncSessionKey) {
                // 记录基线：上游成功后绑定 response.id，下轮同前缀即可增量。
                // 注意：官方 codex 后端已强制 store:false（2026-09-13 实测 400
                // "Store must be set to false"），基线不得改 store——响应可引用性
                // 由后端会话状态保证，路由侧只管 previous_response_id 链式标记
            ...
            for (let officialSendPass = 0; ; officialSendPass += 1) {
              ...
              // 响应头已到：解除断头台，后续由 socket 空闲超时兜底，不干扰长流式。
              if (headDeadlineTimer) { clearTimeout(headDeadlineTimer); headDeadlineTimer = null; }
              if (officialInc && officialSendPass === 0 && upstream.status >= 400 && upstream.status < 500) {                // 拒绝体摘要：区分 previous_response_id 失效/参数不匹配等不同 400 形态
                let incRejectDetail = '';
                try { incRejectDetail = (await readStreamSnippet(upstream.stream, 4096)).slice(0, 300); } catch { /* 已断流 */ }
                officialIncrementalStore.clearSession(officialIncSessionKey);
                flog({
                  event: 'official.incremental_rejected',
                  request_id: requestDiagnostics.requestId,
                  upstream_status: upstream.status,
                  detail: incRejectDetail,
                });
                // 上游不支持 previous_response_id（2026-09-12 移除）→ 连续 3 次后
                // 本进程内自动停用增量，避免每轮白付一次被拒往返（重启后重探）
                if (/previous_response_id|store must be set/i.test(incRejectDetail)) {
                  officialIncrementalRejectStreak += 1;
                  if (officialIncrementalRejectStreak >= 3 && !officialIncrementalDisabled) {
                    officialIncrementalDisabled = true;
                    flog({
                      event: 'official.incremental_disabled',
                      message: '上游连续拒绝 previous_response_id/store 参数，本进程内停用增量续聊（全量直发）',
                    });
                  }
                } else {
                  officialIncrementalRejectStreak = 0;
                }
              ...
              // 增量请求被上游接受：连续拒绝计数清零（间歇性拒绝不应过早熔断）
              if (officialInc && officialSendPass === 0 && upstream.status >= 200 && upstream.status < 300) {
                officialIncrementalRejectStreak = 0;
              }
              break;
            }
            ...
            options.pipeNativeResponse(
              ...
              (response) => {
                ...
                // 仅成功终态可绑定：response.failed/incomplete 的 id 不能作为续聊锚点，
                // 否则下轮增量请求必被拒、落入全量回退（2026-09-13 审计）。
                if (officialIncSessionKey && response?.id && response.status === 'completed') {
              ...
              (bodyText) => {
                    ...
                    // 成功响应（2xx/3xx）不做任何冷却观察：此前 200 也直落到
                    // fallback_429 分支，每个成功请求误发一次诊断、把真实 429 信号
                    // 完全淹没（2026-09-13 对抗审查：全天与 completed 1:1 成对）
            ...
            if (accountEntryId && isAuthOrQuotaFailure(error)) {
              if (error.status === 401) {
                ...
                try {
                  // 429 分型（2026-09-11 修复的 catch 路径补全）：chat/桥接通道抛出的
                  // 429 与 native 路径同用 decide429Cooldown——窗口真实耗尽
                  // （used-percent≥100）才冷却到重置点，突发限流按 retry-after 短冷却。
                  // 旧文本启发式（"rate limit" 字样/retry-after 1-120s）漏判时会把
                  // 有余量账号误锁 30 分钟（2026-09-13 审计）。
                  const decision = decide429Cooldown({
        ...
        stopHeartbeat();
        if (clientClosed || headDeadlineHit) return;
        // 客户端断开与 'close' 监听注册之间的窄窗口可能漏置 clientClosed：
        // 回写前统一以实例属性复核，写已销毁响应会触发 uncaughtException 计入熔断
