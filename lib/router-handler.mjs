        ...
        for (let index = 0; index < compatibleCandidates.length; index += 1) {
          ...
          try {
            ...
            if (upstream.status === 429
              ...
              try {
                // 冷却时长按实际恢复点：429 响应头的额度窗口重置时间 > 缺省 60 秒。
                // 固定 30 分钟会把「短窗限流（并发/突发，额度还剩 90%）」放大成
                // 半小时不可用（2026-09-10 用户实测：有额度却整通道 429 半小时）。
                const limits429 = parseCodexRateLimitHeaders(upstream.headers || {});
                const resetAt429 = limits429 ? (limits429.fiveHour?.resetsAt || limits429.weekly?.resetsAt) : null;
                authManager?.markCooldown?.(attemptKey.entryId, {
                  ...(resetAt429 ? { until: resetAt429 } : { cooldownMs: 60_000 }),
                  reason: '模型额度受限 (429)',
                });
              } catch { /* 冷却失败不影响换号 */ }
            ...
            options.pipeNativeResponse(
              ...
              (bodyText) => {
                    ...
                    // 的同一模型（sub2api 额度池语义）。
                    // 冷却时长按上游额度头的实际重置点；头缺失时短冷却 60 秒——
                    // 固定 30 分钟会把瞬时限流放大成半小时不可用（2026-09-10 实测）。
