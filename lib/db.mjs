...
export function initDatabase(dbPath) {
  ...
  // 补列必须在建表之后：全新库首启时建表自带该列，旧库 ALTER 补列（此前 ALTER
  // 先于建表、靠空 catch 吞 no-such-table 侥幸收敛，是维护地雷）
...
// 单条版保留兼容面（当前全仓零调用方）：转发批量版，date_day/hour_slot 逻辑只此一份。
export function dbRecordTokenLog(log) {
  if (!log || typeof log !== 'object') return;
  dbRecordTokenLogsBatch([log]);
}
...
// ---------- token_logs 批量落库（2026-09-07 并发韧性修复） ----------
// node:sqlite 是同步 API：每条用量记录单独 prepare+insert 都会在事件循环上排队，
// 并发长流下造成流式输出卡顿。批量写入把 N 次 prepare 合并为一次事务。
export function dbRecordTokenLogsBatch(records) {
  ...
  `);
  // 嵌套守卫：node:sqlite 不支持嵌套事务，已处于外部事务时不再 BEGIN/COMMIT，
  // 本批随外层事务一起提交（此时无法单独回滚本批——统计旁路可接受）。
  ...
  try {
    for (const log of records) {
      ...
      stmt.run(
        ...
        new Date(now).toISOString().slice(0, 13) + ':00',
        String(log?.model || 'unknown'),
        log?.target ? String(log.target) : null,
        inputTokens,
        outputTokens,
        Number(log?.reasoningTokens || 0),
        Number(log?.cachedTokens || 0),
        Number(log?.totalTokens || (inputTokens + outputTokens)),
    ...
    if (ownsTransaction) {
      try { db.exec('ROLLBACK'); } catch { /* 事务已回滚或连接不可用 */ }
    }
    throw error;
  }
}

// 内存聚合批量写入器：统计旁路容忍秒级延迟，用微批换取事件循环零碎片阻塞。
// flush 触发条件：定时（默认 5s）/ 满 maxPending 条 / close()（优雅退出）。
// 崩溃窗口最多丢一个 flush 周期的统计（可接受：统计非计费）。
export function createTokenLogBatchWriter({ flushIntervalMs = 5000, maxPending = 200 } = {}) {
  const pending = [];
  let timer = null;
  let closed = false;
  const interval = Math.max(500, Number(flushIntervalMs) || 5000);
  const limit = Math.max(1, Number(maxPending) || 200);
  ...
  const flush = () => {
    ...
    try {
      dbRecordTokenLogsBatch(batch);
    } catch (error) {
      // 统计旁路不得影响路由请求：落库失败只告警，记录不回队（避免死循环堆积）。
      console.warn('[token-usage] 批量落库失败:', error?.message || error);
      return 0;
    }
    return batch.length;
  };
...
export function dbGetDashboardStats(days = 30) {
  ...
  const heatmapGrid = [];
  // 展示标签与数据桶同口径：date_day 是 UTC 日期，标签必须从 dayStr 派生——
  // 本地时区 getMonth/getDate 在 UTC+8 的每天 0-8 点会与 UTC 桶差一天（2026-09-13 审计）
  const utcDisplayDate = (dayStr) => `${Number(dayStr.slice(5, 7))}月${Number(dayStr.slice(8, 10))}日`;
  for (let d = heatmapDays - 1; d >= 0; d--) {
    ...
    heatmapGrid.push({
      date: dayStr,
      displayDate: utcDisplayDate(dayStr),
      tokens: found.tokens,
      rounds: found.rounds,
      level,
    });
  ...
  for (let d = days - 1; d >= 0; d--) {
    const dayObj = new Date(now - d * 86400 * 1000);
    const dayStr = dayObj.toISOString().slice(0, 10);
    // 标签同 UTC 口径（见上方 heatmapGrid 注释）
    const displayLabel = utcDisplayDate(dayStr);
