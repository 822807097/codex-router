...
export function createWebpoolFileBridge({
  ...
  now = () => Date.now(),
  signal = undefined, // 客户端 abort/整体预算（P1-1：上传可被取消）
} = {}) {
  ...
  // P7 二轮 P0 修复：生产注入的 openStream 形状只有 stream/socket（无 bodyText）——
  // 必须回退手工消费，否则三步上传在生产 100% 静默失效；消费后 destroy 防连接滞留
  async function outcomeText(outcome, maxBytes) {
    if (typeof outcome.bodyText === 'string') return outcome.bodyText;
    const src = outcome.stream || outcome.socket;
    if (!src) return '';
    const chunks = [];
    let size = 0;
    try {
      await new Promise((resolve) => {
        ...
        const onData = (c) => { size += c.length; if (size <= maxBytes) chunks.push(c); };
        const cleanup = () => {
          src.off?.('data', onData);
          src.off?.('end', onDone);
          src.off?.('error', onDone);
          src.off?.('close', onDone);
        };
        src.on('data', onData);
        src.once('end', onDone);
        src.once('error', onDone);
        src.once('close', onDone);
      });
    } finally {
  ...
  async function callJson({ path, method = 'POST', body = null, timeout }) {
    ...
    const text = await outcomeText(outcome, 256 * 1024);
    let parsed = null;
  ...
  async function putBlob(uploadUrl, bytes, mime) {
    ...
    await outcomeText(outcome, 16 * 1024); // 消费并释放连接（PUT 无需响应体）
    if (outcome.status < 200 || outcome.status >= 300) {
