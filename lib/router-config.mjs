...
function inspectOfficialIncremental(config, errors) {
  if (config.officialIncremental === undefined || config.officialIncremental === null) return;
  if (config.officialIncremental === false) return;
  if (!plainObject(config.officialIncremental)) {
    addInvalid(errors, 'official_incremental_invalid', '/officialIncremental', '必须是普通对象或 false');
    return;
  }
  if (
    configuredValue(config.officialIncremental, 'enabled')
    && config.officialIncremental.enabled !== null
    && typeof config.officialIncremental.enabled !== 'boolean'
  ) {
    addInvalid(errors, 'official_incremental_invalid', '/officialIncremental/enabled', '必须是 boolean');
  }
  for (const field of ['maxEntries', 'maxBytes']) {
    if (
      configuredValue(config.officialIncremental, field)
      && config.officialIncremental[field] !== null
      && normalizedInteger(config.officialIncremental[field]) === null
    ) {
      addInvalid(errors, 'official_incremental_invalid', `/officialIncremental/${field}`, '必须是正整数');
    }
  }
}
...
function inspectRouterConfigState(config, context = {}) {
  ...
  inspectOfficialIncremental(config, errors);
