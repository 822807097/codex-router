...
// ---------- 运行时缓存 ----------
// 双轨：compact（文本协议提示词用，省 token）+ raw（原生 MCP 门面 tools/list 用，全信息量）
const runtimeCatalogs = new Map(); // serverKey -> [{name, description, parameters}] 压缩形态
const runtimeRawCatalogs = new Map(); // serverKey -> [{name, description, inputSchema}] 真实形态
const refreshInFlight = new Map(); // serverKey -> Promise（并发触发合并，P2-9）
let refreshStatus = {
...
/** 刷新目录：拉真实 tools/list 成功即替换缓存；失败保留旧值并记录错误；并发合并。 */
export function refreshWebpoolToolCatalog(serverName = CU_SERVER_NAME, spec = null, { probe = probeMcpServerFull } = {}) {
  const existing = refreshInFlight.get(serverName);
  ...
  const run = (async () => {
    ...
    runtimeRawCatalogs.set(serverName, probed.tools
  ...
  })().finally(() => { refreshInFlight.delete(serverName); });
  refreshInFlight.set(serverName, run);
  return run;
}
...
/** 原生门面向：真实未压缩 schema（P2-7）；无 runtime 时回退压缩形态。 */
export function getWebpoolToolCatalogNative(serverName = CU_SERVER_NAME) {
  const raw = runtimeRawCatalogs.get(serverName);
  if (raw?.length) return raw;
  return getWebpoolToolCatalog(serverName).map((entry) => ({
    name: entry.name,
