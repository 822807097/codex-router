...
// ---------- SOCKS5 客户端原生轻量协议握手 (RFC 1928, 认证代理走 RFC 1929) ----------
export function handshakeSocks5(socket, targetHost, targetPort, credentials = null) {
  ...
  if (useAuth && (Buffer.byteLength(username, 'utf8') > 255 || Buffer.byteLength(password, 'utf8') > 255)) {
    return Promise.reject(new Error('SOCKS5 用户名/密码超过 255 字节（RFC 1929 上限）'));
  }
  return new Promise((resolve, reject) => {
    // 阶段：0=认证方式协商（支持账密） 1=用户密码子协商（RFC 1929） 2=CONNECT 响应
    // 免账密时跳过子协商：0.5=认证方式协商（仅 NO_AUTH）后直接 CONNECT。
    let stage = useAuth ? 0 : 0.5;
    ...
    const onData = (chunk) => {
      ...
      try {
        if (stage === 0 || stage === 0.5) {
          // 期望响应: 0x05 (VER) METHOD
          if (buffer.length < 2) return; // 等待响应完整到达
          const method = buffer[1];
          if (buffer[0] !== 0x05 || (stage === 0.5 && method !== 0x00)) {
            cleanup();
          ...
          if (method === 0x00) {
            sendConnect();
          } else if (stage === 0 && method === 0x02) {
            // RFC 1929 用户名/密码子协商：VER=1 ULEN USER PLEN PASS
            stage = 1;
            const user = Buffer.from(username, 'utf8');
            const pass = Buffer.from(password, 'utf8');
            const req = Buffer.alloc(3 + user.length + pass.length);
            req[0] = 0x01;
            req[1] = user.length;
            user.copy(req, 2);
            req[2 + user.length] = pass.length;
            pass.copy(req, 3 + user.length);
            socket.write(req);
          } else {
            // 0xFF（无可用方式）或服务器强制选择了未提议的方式
            cleanup();
            reject(new Error(useAuth
              ? 'SOCKS5 代理不支持用户名密码认证（0xFF 无可用认证方式）'
              : `SOCKS5 认证协商失败: ${method.toString(16)}`));
          }
        } else if (stage === 1) {
          // 期望响应: 0x01 (VER) STATUS，STATUS=0 成功
          if (buffer.length < 2) return;
          if (buffer[0] !== 0x01 || buffer[1] !== 0x00) {
            cleanup();
            reject(new Error('SOCKS5 用户名密码认证失败（账号或密码错误）'));
            return;
          }
          // 保留子协商响应之后的残留字节（服务器流水线发送 CONNECT 响应时不能丢）
          buffer = buffer.subarray(2);
          sendConnect();
        } else {
    ...
    // 发起初次协商：带账密时提供 NO_AUTH + USERPASS 两种方式，由服务器选择
    socket.write(useAuth
      ? Buffer.from([0x05, 0x02, 0x00, 0x02])
      : Buffer.from([0x05, 0x01, 0x00]));
  });
}

// 解析代理 URL（http/socks5，host:port 简写兼容；userinfo 形态的账号密码一并提取），非法返回 null
...
function proxyAuthorizationHeader(parsedProxy) {
  if (!parsedProxy?.username && !parsedProxy?.password) return null;
  const token = Buffer.from(`${parsedProxy.username || ''}:${parsedProxy.password || ''}`, 'utf8').toString('base64');
  return `Basic ${token}`;
}
...
export function connectTls(options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (parsedProxy.protocol.startsWith('socks')) {
      // SOCKS5 原生握手（认证代理走 RFC 1929 用户名密码子协商）
    ...
    // HTTP CONNECT 原生握手
    const proxyAuth = proxyAuthorizationHeader(parsedProxy);
...
function poolPark(socket, key, eligible) {
  ...
  // setTimeout(idleMs, cb) 是「追加」一次性 timeout 监听：不清掉会随复用次数累积
  // （MaxListenersExceededWarning: 11 timeout listeners 实证）。data/error/end/close
  // 已 removeAllListeners，timeout 此前漏了。
