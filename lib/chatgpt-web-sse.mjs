...
export function createConversationToChatTransform({ model, tools = null, onTextFinal = null, log = null } = {}) {
  ...
  return new Transform({
    ...
    flush(cb) {
      try {
        ...
        // 原生模式守卫：把累计全文交回通道（空回答/工具不可用泄漏 → 会话降级）
        if (typeof onTextFinal === 'function') { try { onTextFinal(fullText); } catch { /* 旁路 */ } }
        // 工具桥：流末对全文跑一次协议解析；命中→吞正文发伪 tool_calls
      if (hasTools) {
        const parsed = parseToolCallBlocks(fullText, allowedNames);
        const { calls, cleanedText } = parsed;
        if (calls.length) {
            if (!roleEmitted) {
              roleEmitted = true;
              this.push(chatChunkFrame({ id, model, created, role: 'assistant' }));
          ...
          const { verdict } = extractVerdict(cleanedText);
          if (verdict && typeof log === 'function') {
            try { log({ event: 'chatgpt_web.verdict', outcome: String(verdict).slice(0, 80) }); } catch { /* 旁路 */ }
          }
          const visible = sanitizeBridgeOutput(cleanedText);
          const text = visible || (parsed.hadBlock && parsed.malformedBlocks > 0
            ? `（上游工具调用块格式损坏（${parsed.malformedBlocks} 处），已忽略；任务可能未完成，请重试或换个说法）`
            : '');
          if (text) {
            if (!roleEmitted) {
              roleEmitted = true;
            ...
            this.push(chatChunkFrame({ id, model, created, content: text }));
          }
        }
        if (!roleEmitted) {
          this.push(chatChunkFrame({ id, model, created, role: 'assistant' }));
...
export function conversationSseToChatCompletion(bodyText, { model, tools = null, onTextFinal = null } = {}) {
  ...
  // 原生模式守卫（非流式路径同语义）：净化前的全文交回通道做降级判定
  if (typeof onTextFinal === 'function') { try { onTextFinal(content); } catch { /* 旁路 */ } }
  const allowedNames = new Set(
    (Array.isArray(tools) ? tools : [])
      .map((t) => (t?.type === 'function' && t.function?.name ? t.function.name : null))
      .filter(Boolean),
  ...
  if (allowedNames.size) {
    const parsed = parseToolCallBlocks(content, allowedNames);
    const { calls, cleanedText } = parsed;
    if (calls.length) {
      return {
        ...base,
    ...
    // 兜底必须落到 ''（与流式路径一致）：回退原文会把未净化的协议块/注入回声
    // 泄给客户端（2026-09-13 对抗审查复现：空 name 块、纯回声场景）
    content = sanitizeBridgeOutput(cleanedText)
      || (parsed.hadBlock && parsed.malformedBlocks > 0
        ? `（上游工具调用块格式损坏（${parsed.malformedBlocks} 处），已忽略；任务可能未完成，请重试或换个说法）`
        : '');
  } else {
