...
// ---------- 余量感知选号（借鉴 sub2api openAIQuotaHeadroomFactor，2026-09-11） ----------
// 从响应头捕获的额度快照（metadata.rateLimits，account-quota.mjs 写入）估算账号
// 剩余额度因子 0..1：长窗（周/主窗）余量为主，短窗（5h）余量 <10% 时减半——
// 临近小时窗重置的账号先歇一歇。快照缺失或超过 8 小时视为中性 0.5，不参与偏好。
const HEADROOM_SNAPSHOT_STALE_MS = 8 * 60 * 60_000;
const HEADROOM_NEUTRAL = 0.5;
const HEADROOM_SHORT_LOW_REMAIN = 0.10;

export function quotaHeadroomFactor(rateLimits, nowMs = Date.now()) {
  if (!rateLimits || typeof rateLimits !== 'object') return HEADROOM_NEUTRAL;
  const updatedAt = Number(rateLimits.updatedAt);
  if (!Number.isFinite(updatedAt) || nowMs - updatedAt > HEADROOM_SNAPSHOT_STALE_MS || nowMs < updatedAt) {
    return HEADROOM_NEUTRAL;
  }
  const windows = [rateLimits.fiveHour, rateLimits.weekly]
    .filter((w) => w && Number.isFinite(w.usedPercent))
    .map((w) => ({ minutes: Number(w.windowMinutes) || 0, used: Math.max(0, Math.min(100, w.usedPercent)) }));
  if (!windows.length) return HEADROOM_NEUTRAL;
  const long = windows.reduce((a, b) => (b.minutes > a.minutes ? b : a));
  let factor = 1 - long.used / 100;
  const shorts = windows.filter((w) => w !== long);
  if (shorts.length) {
    const short = shorts.reduce((a, b) => (b.minutes < a.minutes ? b : a));
    if (1 - short.used / 100 < HEADROOM_SHORT_LOW_REMAIN) factor *= HEADROOM_NEUTRAL;
  }
...
export function createAuthManager(options = {}) {
  ...
  // 仅接受 cooldown/suspect：auth_expired 是凭据被吊销态，强行复活只会
  // 刷新再 401 循环（应走重新授权）。
  function clearCooldown(id) {
    const acc = accounts.get(id);
    if (!acc) return false;
    if (acc.status !== 'cooldown' && acc.status !== 'suspect') return false;
    acc.status = 'active';
    acc.cooldownUntil = 0;
    try {
  ...
  function acquireAccount(providerOrOptions) {
    ...
    // 消耗顺序：先剔除已耗尽账号（余量因子为 0 且存在有余量同伴时让位，避免
    // 磨死一个干涸账号而放弃健康的低优先级账号），再按账号显式优先级（面板可设）
    // > 周窗余量（降序）选择；余量并列时轮询均摊。快照缺失/超 8h 按中性 0.5，
    // 不会因"未知"被饿死（sub2api openAIQuotaHeadroomFactor 同款思路，2026-09-11）。
    // 套餐对模型可用性的约束由 accountSupportsModel 过滤承担。
    const priorityRank = (acc) => {
      const raw = acc.metadata?.priority;
      const num = Number(raw);
      return (raw !== undefined && raw !== null && raw !== '' && Number.isFinite(num)) ? num : 50;
    };
    const headroomOf = new Map();
    for (const acc of activeList) {
      headroomOf.set(acc, quotaHeadroomFactor(acc.metadata?.rateLimits, now()));
    }
    let pool = activeList;
    const withHeadroom = activeList.filter((acc) => (headroomOf.get(acc) || 0) > 0);
    if (withHeadroom.length > 0 && withHeadroom.length < activeList.length) {
      pool = withHeadroom;
    }
    const leadPriority = Math.min(...pool.map((acc) => priorityRank(acc)));
    const leadClass = pool.filter((acc) => priorityRank(acc) === leadPriority);
    if (leadClass.length === 1) return leadClass[0];
    const best = Math.max(...leadClass.map((acc) => headroomOf.get(acc)));
    const ties = leadClass.filter((acc) => Math.abs(headroomOf.get(acc) - best) < 1e-9);
    if (ties.length === 1) return ties[0];
    const ptr = rrPointers.get(p) || 0;
    rrPointers.set(p, (ptr + 1) % ties.length);
    return ties[ptr % ties.length];
  }
