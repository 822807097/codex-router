...
export function createAuthManager(options = {}) {
  ...
  function acquireAccount(providerOrOptions) {
    ...
    // > 周窗余量（降序，sub2api openAIQuotaHeadroomFactor 同款思路，2026-09-11 借鉴：
    //   主动打余量最肥的号，比 429 后被动换号的额度利用率高得多）> 轮询均摊。
    // 套餐对模型可用性的约束由 accountSupportsModel 过滤承担；余量并列时跨套餐轮询。
    const priorityRank = (acc) => {
      const raw = acc.metadata?.priority;
      const num = Number(raw);
      return (raw !== undefined && raw !== null && raw !== '' && Number.isFinite(num)) ? num : 50;
    };
    const leadPriority = priorityRank(activeList.reduce((a, b) => (
      priorityRank(a) <= priorityRank(b) ? a : b)));
    const leadClass = activeList.filter((acc) => priorityRank(acc) === leadPriority);
    if (leadClass.length === 1) return leadClass[0];
    const headrooms = leadClass.map((acc) => ({
      acc,
      headroom: quotaHeadroomFactor(acc.metadata?.rateLimits, now()),
    }));
    const best = Math.max(...headrooms.map((h) => h.headroom));
    const ties = headrooms.filter((h) => Math.abs(h.headroom - best) < 1e-9);
    if (ties.length === 1) return ties[0].acc;
    const ptr = rrPointers.get(p) || 0;
    rrPointers.set(p, (ptr + 1) % ties.length);
    return ties[ptr % ties.length].acc;
  }
