...
| **ChatGPT 网页池原生工具模式（实验）** | 网页池账号可改走官方连接器 + 安全隧道的原生工具通道：联网搜索 / 电脑操作以真·工具调用直达网页会话，支持截图回注与按账号开关；默认关闭，文本协议链路始终自动兜底（详见 [docs/ADVANCED.md](docs/ADVANCED.md)） |
...
| 桌面端 MCP 插件管理 | 面板直接管理 Codex 桌面端原生插件（`config.toml` 的 `[mcp_servers.*]`）：列表 / 启停 / 连接测试 / 增删改，兼容 codex++ 生态插件的 MCP 部分——例如让任何模型都能操作电脑的 Windows 电脑控制插件 |
| 代理逐通道配置 | 每通道可直连 / 走全局代理 / 走自定义节点（支持粘贴 ss / trojan / vless / socks5 / http 链接，**支持带账号密码的认证代理**，SOCKS5 与 HTTP 认证均已打通） |
...
- **桌面端 MCP 插件管理**：设置页「桌面端 MCP 插件」卡片，管理桌面端原生插件（`config.toml` 的 `[mcp_servers.*]` 段）——启停（官方 `enabled` 键，停用不删配置）、连接测试（真实拉起插件并列出工具清单）、增删改；改完重启桌面端生效。详见 [docs/ADVANCED.md](docs/ADVANCED.md)。
- **路由内置工具桥**：网页池/第三方模型没有官方服务端工具时，路由可代为注入并执行联网搜索、MCP 工具与技能（`config.json` 的 `tools` 段，详见 ADVANCED）。
...
**Q：代理连不上 / 需要全局代理？**
见 [docs/ADVANCED.md「网络与代理」](docs/ADVANCED.md)。逐通道可配代理，自定义代理支持直接粘贴机场节点链接（ss / trojan / vless / socks5 / http）；带账号密码的认证代理也已支持（socks5://user:pass@host:port 形态，或在面板里分字段填写）。

**Q：网页池的「原生工具模式」是什么？为什么默认关闭？怎么启用？**
