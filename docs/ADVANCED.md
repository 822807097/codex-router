...
## 网页池原生工具模式（WebPool 2.0，实验）

一句话架构：**ChatGPT 网页连接器 → 官方安全 MCP 隧道 → 本机隧道客户端（路由自动托管）→ 路由门面（127.0.0.1）→ 现有工具执行面**。开启后，网页池账号的联网搜索 / 电脑操作不再靠文本协议「翻译」，而是以真·工具调用经官方隧道直达网页会话；工具过程产生的截图还会自动回注进对话，让模型「看得见」自己做了什么。

> 适合谁：想让网页池模型拥有更可靠的联网搜索与电脑操作能力、且愿意做一次人工配置的进阶用户。全程只多开一个本机端口（默认 15731），隧道为纯出站连接，不需要在路由器上开任何入站端口。
...
| 键 | 作用 | 默认 |
| --- | --- | --- |
| `enabled` | 原生工具模式总开关 | `false` |
| `port` | 本机门面监听端口 | `15731` |
| `exposeComputer` | 是否暴露电脑操作工具 | `true` |
| `exposeWebSearch` | 是否暴露联网搜索工具 | `false` |
| `bearerKey` | 门面 Bearer 口令读取的**环境变量名**（值本身不写进配置） | `ROUTER_MCP_BEARER` |
| `tunnel.enabled` | 是否托管官方隧道客户端（自动下载/启动/守护） | `false` |
| `tunnel.channel` | 隧道通道名 | `main` |

### 环境变量

| 变量 | 作用 | 说明 |
| --- | --- | --- |
| `ROUTER_MCP_BEARER` | 本机门面 Bearer 口令 | **fail-closed**：未设置时门面直接拒绝监听（防止裸奔端口），面板会提示先配好再开 |
| `CONTROL_PLANE_TUNNEL_ID` | 平台 Tunnel ID | 格式 `tunnel_` + 32 位十六进制，格式不对隧道不会启动 |
| `CONTROL_PLANE_API_KEY` | 平台 Runtime Key | 建议只授予 Tunnels Read + Use 权限；只进环境变量，不写配置文件 |

### 管理 API（面板与脚本用）

| 端点 | 用途 |
| --- | --- |
| `GET /_admin/api/webpool/native-status` | 原生模式总览：全局/逐账号开关、门面与隧道状态、凭据变量名（值永不出面） |
| `GET /_admin/api/webpool/metrics` | 指标聚合：原生/协议/降级/裁决/上传与门面调用计数，按日聚合 |
| `POST /_admin/api/webpool/native-selftest` | 一键自探：逐段检查门面 → 隧道 → 账号链路 |
| `POST /_admin/api/webpool/accounts/native-tools` | 逐账号开关原生模式（写账号元数据并持久化，重启不丢） |

### 行为语义

- **逐账号开关**：原生模式按账号启用，只对显式开启且全局开关打开的账号生效；其余账号照走文本协议。
- **自动降级**：原生回答为空或泄漏「工具不可用」类字样时，该会话自动降级回文本协议并保持 **30 分钟**（TTL 到期自动重试原生），任务不中断。
- **截图回注**：电脑操作等工具产生的截图先上传、再以官方图片指针回注进会话；上传失败自动回落纯文本描述，不会因此报错。
- **电脑操作单飞队列**：桌面是单一资源，电脑工具调用排队单飞执行（超时放弃不破坏互斥），避免并发操作互相踩踏。

### 安全边界

- **门面**：仅监听 127.0.0.1 回环地址；强制 JSON content-type、拒绝带 Origin 的请求（浏览器 CSRF 防线）；叠加 Bearer 口令校验。
- **隧道**：使用官方隧道客户端，纯出站连接；托管下载走 HTTPS + 域白名单 + IP 字面量拒绝（环回/私有/保留段全拒）。
- **下载校验**：客户端压缩包按官方 `SHA256SUMS.txt` 逐字节校验，缓存命中复算哈希比对，篡改即拒绝执行。
- **日志脱敏**：隧道进程 stderr 输出先做密钥剥离（Bearer 值、平台 Key、URL key 段、长 base64 串统一打码）再进日志。
